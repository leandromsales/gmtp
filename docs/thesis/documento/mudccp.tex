...
precedência entre dois elementos diferentes.
  ...
  \item Sejam as relações e restrições estabelecidas entre os diferentes tipos
de nós de uma transmissão de um evento ao vivo \event, definida por
\transmission $= \{$\setserv, \setrepass, \subsetcli, \setpk$\}$, tal que:

\begin{itemize}

  \item Seja uma função \subsetclis que denota todos os clientes relacionados a
um nó \repass, de modo que nenhum nó \clis $\in$ \setclis pode estar relacionado
com dois ou mais nós, portanto definida por \subsetclis $:$ \repasss
$\rightarrow$ \powerset{\setclic},
$\forall$\repassu{\repassi_{1}},\repassu{\repassi_{2}} $\in$
\setrepass, \subsetcli$($\repassu{\repassi_{1}}$)$ $\cap$
\subsetcli$($\repassu{\repassi_{2}}$)$ = $\{\emptyset\}$;

  \item $($\setpk$,\prec) = \{\pkconst_{1},\pkconst_{2}, \pkconst_{3}, \ldots,
$\pku{x}$\}$, tal que $x \in \mathbb{N}$. Isto é, o conjunto
\textit{parcialmente ordenado (poset)} dos pacotes de dados \pks transmitidos
por um nó \servs ou por um nó \repass, também chamado de fluxo de pacotes de
dados ou apenas fluxo de dados.
...
\end{itemize}
  ...
  \item Seja o conjunto finito de todos os nós relatores de uma transmissão
\transmission, definido por \setrels $= \{\relconst_1, \relconst_2, \ldots,
$\rel$\}$, de modo que qualquer nó \clis $\in$ \subsetcli$($\repass$)$
pode atuar como um nó relator \rels de um nó repassador \repass. Assim,
para um subconjunto de nós \rels de \repass, definido por \subsetrels $\in$
\powerset{\setclic_\subsetclii(\repassconst_\repassi)}, tem-se \rels $\in$
\subsetrels tal que \subsetcli$($\repass$)$  $\cup$ \subsetrels $ = $
\subsetcli$($\repass$)$.
  \label{i:cons-mcc-ri}
...
por \setwayif, se for \setwayisfs e se \setwayis $\subset$ \settrans, tal que
\settranss $\subset$ \setservrepasss é o conjunto dos nós que transmitem ou
repassam os pacotes de dados \pks $\in$ \setpk, definido por \settranss $=
\{$\transs$\mid$ \transmitqus{\trans} $ = 1\}$, sendo $\transi \in \mathbb{N}$ e
$\varphi$ uma função booleana que determina se um nó \transs $\in$ \settranss
transmite os pacotes \pks $\in$ \setpks de um evento \events para \trans, ou
para pelo menos um \clis $\in$ \subsetcli(\trans), ou seja:

      \begin{itemize}

	\item $\varphi:$ (\trans, \setpk) $\rightarrow \{0, 1\}, \forall
($\trans, \setpk$) \in \{$\settrans$ \times \{$\setpk$\}\}$, onde 0 e 1 denotam,
...
\invert{\setwayi} $ = \{$\ways $\mid$ \repass
...
redes de computadores, um nó \netrs $\in$ \setnetr, portanto um nó \repass,
fica indisponível com menos frequência se comparado aos seus nós
\subsetcli(\repass). Nas transmissões de dados na Internet, a participação de um
roteador no processo de transmissão de um fluxo de dados qualquer é fundamental,
mesmo que seja apenas para rotear os pacotes de um fluxo de dados. Apesar
de óbvia, tal observação é importante porque para um nó qualquer \clis receber
um determinado fluxo de dados \setpk, primeiramente os pacotes de dados de tal
fluxo deve passar pelo roteador padrão de \cli. Sendo assim, quando um nó
\repass se desconecta, nenhum de seus nós \subsetcli(\repass) será capaz de
receber \setpk, mas a recíproca não é verdadeira. Especificamente,
pretende-se permitir que, mesmo se um nó \clis se desconectar, outros nós \clis
possam continuar recebendo o fluxo de dados \setpk, pois este será encaminhado
por \repasss e não mais por qualquer nó \cli, como acontece em soluções
tradicionais de distribuição de conteúdos multimídia baseado em uma arquitetura
P2P;

    \item \textit{Conjectura 2}: as tabelas de roteamento dos nós \ways $\in$
\setwayis não mudam frequentemente e são independentes umas das outras. No mundo
real (Internet, por exemplo), as rotas entre quaisquer nós \cliu{\clii_1} e
\cliu{\clii_2} $\in$ \setclis não se alteram com um nível de frequência que
desestabilize a comunicação entre estes, apesar de se tratar de uma rede
comutada por datagramas IP. Mesmo se estas mudanças ocorrerem em uma rota de um
caminho \setwayi, o impacto causado é temporário e insignificante no ponto de
vista da experiência do usuário ao assistir a um evento \event. Com isto, é
possível antecipar a formaçào de parcerias, ou seja, caminhos \setwayis antes
da efetiva transmissão de um fluxo de dados \setpk.
...
pelo GTMP, definindo-se as relações e restrições
estabelecidas em \transmissions e as conjecturas consideradas para a execução de
tal protocolo. Sendo assim, a Figura~\ref{fig:cenario-global-detailed} pode ser
refeita como ilustratada na Figura~\ref{fig:cenario-global-detailed-formal}.

FAZER UMA FIGURA COM A SOBREPOSIÇÃO E UM FUNDO SEMI-TRANSPARENTE
...
conteúdos multimídia, que envolvem os nós em \setservrepasss e suas funções
específicas.
...
campo de $RTT_{s}$. Quando \ways receber um pacote \pac{GMTP-Ack}, deve-se
...
os nós parceiros de \repass, pois teoricamente estes não poderão mais receber
os pacotes de dados \pks $\in$ \setpk. Para evitar um período de instabilidade
na recepção de \setpk por parte dos nós parceiros de \repass, o GMTP define um
parâmetro chamado de período de carência para novas parcerias (\textit{grace
period for new partnerships}). Este parâmetro determina o tempo que um nó
\repasss continuará repassando o fluxo de dados \setpks para seus parceiros. O
valor para esse tempo é transmitido para os nós parceiros de \repass, que por
sua vez deve iniciar o procedimento de realizar outras parcerias a fim de
continuar recebendo o fluxo de dados \setpk. Opcionalmente, um nó \repasss pode
aceitar receber de seus nós parceiros tal valor para o referido parâmetro,
desde que não ultrapasse um limite máximo definido pelo administrador de
\repass.
