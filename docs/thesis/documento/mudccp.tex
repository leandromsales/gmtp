...
Note que, no GMTP, toda transferência de pacotes de controle entre nós \repass ocorre com
...
Para finalizar esse assunto, salienta-se que o registro de participação do GMTP
...
No GMTP, dado um nó \repass, a seleção de nós parceiros \repassu{q} ocorre
antes e durante a transmissão de um fluxo de dados \setpk, através dos
procedimentos de formação de parcerias discutidos nessa seção, que basicamente
consiste em formar intersecções de caminhos utilizados para transmitirq \setpk.
A formação da parceria ocorre de forma transparente durante o pedido de
conexão de um nó \clis ao nó \servs para obter \setpk. A formação de parcerias
resulta na constituição de um ou mais caminhos \setwayis
$\in$ \setway, que interligará um nó \servs a um ou mais nós \clis $\in$
\subsetcli(\repass). Sendo assim, qualquer nó \ways $\in$ \setwayis pode agir
como se fosse um nó \serv.
...
Na Figura~\ref{fig:esquema-abstrato-formacao-parceria-intra}, ilustra-se um
exemplo para entender o processo de formação de parceria. No Passo 1, ilustra-se
um cenário de rede \nets com 19 nós \repasss e dois nós \serv, sem qualquer
fluxo de dados \setpks sendo transmitido, tampouco nenhuma parceria efetivada e
suprimindo-se os nós \cli. No Passo 2, ilustra-se a transmissão de um fluxo
de dados \setpks que ocorre entre os nós \servu{1} e \repassu{9},
constituindo-se o caminho \setwayiu{1} (linha tracejada e vermelha). Para
constituir o caminho \setwayiu{1}, o nó \repassu{9} transmite o pedido de
registro de participação ao nó \servu{1}
(Seção~\ref{subsec:registro-participacao}) e, a partir de sua confirmação, o nó
\repassu{9} começa a receber os pacotes \pks $\in$ \setpk. Apesar de um
procedimento simples, o nó \servu{1} passa a conhecer o caminho \setwayiu{1}, a
partir do nó \repassu{9}, sendo tal conhecimento importante para a formação de
futuras parcerias. Este cenário será utilizado como base para todos os exemplos
apresentados a seguir.

Dado o cenário base ilustrado anteriormente, considere a
Figura~\ref{fig:esquema-abstrato-formacao-parceria-interseccao}, onde ilustra-se
um exemplo de formação de parcerias. Considera-se a formação de parceria por
intersecção do fluxo de dados \setpk, que ocorre quando um outro nó \repasss
$\in$ \setwayiu{1} envia um pedido de registro de participação ao nó \servu{1},
a fim de obter o fluxo de dados \setpk, motivado por algum nó \clis $\in$
\subsetcli$($\repass$)$. Nesse caso, se o pedido de registro de participação for
transmitido através de um sub-caminho \setwayidu{1} $\subset$ \setwayiu, o nó
\servs determina a intersecção de ambos e continua-se transmitindo apenas um
único fluxo de dados \setpk. Este procedimento então se reduz à necessidade de
identificar um nó \ways comum a dois ou mais caminhos \setwayi, permitindo-se
que o referido nó \ways replique o fluxo de dados \setpk, mesmo quando
$\mid$\subsetcli$($\way$)\mid$ $=$ $0$, mas que seja conveniente para evitar
múltiplas transmissões do fluxo de dados \setpk, originadas no nó \serv. Para
entend r melhor este caso, acompanhe a explicação a seguir com base na
ilustração da
Figura~\ref{fig:esquema-abstrato-formacao-parceria-interseccao} e no caminho
\setwayiu{1}.
...
\repassu{11}, enviar um registro de participação (Passo 2 da
...
de participação), ou automaticamente repassará o fluxo de dados \setpks para
\repassu{11}, caso \repassu{11} envie um registro de participação sinalizando
interesse em obter \setpks (Passo 3). Isto ocorrerá porque, com o registro de
participação de \repassu{11}, o nó \servs descobrirá \setwayiu{2} $=$
$\{$\repassu{11}, \repassu{10}, \repassu{7}, \repassu{6}, \repassu{5}$\}$ e,
pela intersecção $($\setwayiu{1} $\cap$ \setwayiu{2}$)$, descobrirá o nó
\repassu{7} comum entre ambos os caminhos (Passo 4). Especificamente, isto
ocorre porque o nó \servu{1} é capaz de identificar que \setwayidu{1} $=$
\setwayidu{2} =
\invert{$\delta($\repassu{7}, \setwayiu{1}$)$} $\cup$
\invert{$\delta($\repassu{7}, \setwayiu{2}$)$} = $\{$\repassu{7}, \repassu{6},
\repassu{5}, \servu{1}$\}$.

Como resultado dessa intersecção, deve-se determinar que
\transmitqu{\repassu{7}} $=$ $1$. Em termos práticos, isto obriga o nó
\repassu{7} a adicionar uma nova entrada na tabela de recepção de fluxos de
dados referente a \setpk, mesmo se $\mid$\subsetcli$($\repassu{7}$)\mid$ $=$
$0$ com relação a \setpk. Obviamente, se posteriormente
$\mid$\subsetcli$($\repassu{7}$)\mid$ $>$ $0$ com relação a \setpk, quando um
ou mais nós \clis $\in$ \subsetcli$($\repassu{7}$)$ enviar um pedido de conexão
para obter o fluxo de dados \setpk, será necessário apenas que \repassu{7} crie
um canal multicast para transmissão local, evitando-se um novo registro de
participação em \servu{1}. Este aspecto do GMTP será discutido em mais detalhe
na Seção~\ref{subsec:conexao-requisicao}.

Ao estender a discussão sobre o exemplo ilustrado na
...
procedimento supracitado se repete. Uma situação similar também ocorrerá
se em vez do nó \repassu{10} forem os nós \repassu{12} (Passo 5) ou qualquer nó
\repasss $\in$ \setwayiu{4}, tal que \setwayiu{4} $=$ $\{$\repassu{1},
\repassu{2}, \repassu{3}, \repassu{4}$\}$. Se o nó \repassu{12} desejar obter
\setpk, o nó \repassu{11} interceptará o pedido de registro de participação, ao
passo que se for qualquer nó \repasss $\in$ \setwayiu{4}, o nó \repassu{6}
realizará tal interceptação, mas antes o nó \servu{1} determinará
\transmitqu{\repassu{6}} $=$ $1$ -- similar ao ocorrido anteriormente com o nó
\repassu{7}. A única diferença nesses últimos casos é que, como
\transmitqu{\repassu{7}} $=$ $1$ e \transmitqu{\repassu{11}} $=$ $1$,
\repassu{7} tem autonomia para responder ao nó \repassu{10} e o nó \repassu{11}
tem autonomia para responder ao nó \repassu{12}, como se fossem o nó \servu{1},
sem repassar tal pedido em direção ao nó \servu{1}. Isto também ocorrerá com o
nó \repassu{6}, porém depois do primeiro pedido de registro de participação
originado por qualquer nó \repasss $\in$ \setwayiu{4} (Passo 6).
...
dados \setpks e que estejam interligados direto ou indiretamente a \repass, tal
que \transmitqu{\repass} $=$ $1$, o nó \repasss sempre interceptará o pedido de
registro de participação e agirá como se fosse o nó \servu{1}. No
caso do exemplo anterior, independente da ordem em que as requisições de
registro de participação sejam enviadas por \ways $\in$ (\setwayiu{1} $\cup$
\setwayiu{2} $\cup$ \setwayiu{3} $\cup$ \setwayiu{4}), transmite-se apenas um
fluxo de dados \setpks para ``alimentar'' os quatro caminhos referidos. Isto
significa que todos os nós \clis $\in$ \subsetcli$($\setwayiu{1} $\cup$
\setwayiu{2} $\cup$ \setwayiu{3} $\cup$ \setwayiu{4}$)$ receberão um único fluxo
de dados, com repasse dos pacotes \pks $\in$ \setpks realizado em modo multicast
para cada sub-rede representada por \ways (Passo 7).

Note que, todo nó \repasss que interceptar um pedido de conexão para um fluxo de
dados \setpk, deve transmitir para o nó \servs uma notificação sobre tais
parcerias formadas por intersecção. No caso do exemplo anterior, os nós
\repassu{6}, \repassu{7} e \repassu{11} deve enviar um pacote do tipo
\pac{GMTP-Register}, como explicado na Seção~\ref{subsec:registro-participacao}.
Esta ação é importante devido aos aspectos gerenciais de uma transmissão, pois
permite-se que a aplicação em execução no nó \servs possa contabilizar os nós
\repasss que estão recebendo \setpk, mesmo que indiretamente. No ponto de vista
prático, não se faz necessário que o nó \repasss envie tal notificação no
instante que interceptar um pedido de registro de participação. Em vez disso,
pode-se acumular diversos registros de participação durante um determinado
intervalo de tempo e apenas em seguida transmiti-los para o nó \serv. Visto que
se trata de uma discussão a nível de implementação, tal decisão está fora do
escopo deste trabalho. Para simplificar a implementação do GMTP utilizado neste
trabalho, definiu-se que para todo registro de participação interceptado,
automaticamente gera-se uma notificação ao nó \serv.

No Algoritmo~\ref{algo:findPartnerIntersectPath}, apresenta-se os passos
...
para obter um fluxo de dados \setpk. O nó \servs compara os caminhos conhecidos
...
caso contrário, \servs aceita o novo pedido de conexão e todo o caminho
\textit{\setwayiu{\repassconst_\repassi}} passa a ser alimentado com o fluxo de
dados \setpk, permitindo-se futuras parcerias por intersecção. Note que, após
este procedimento, o nó \servs passa a conhecer o caminho
\textit{\setwayiu{\repassconst_\repassi}}.

O algoritmo explicado anteriormente permite repasse de pacotes de dados \pks
levando em consideração o fluxo de dados de interesse \setpk, portanto trata-se
de uma abordagem centrada no conteúdo \textit{(content centric networks)}, e não
centrada no sistema final \textit{(host centric networks)}. Essa proposta quebra
um paradigma que tradicionalmente os pacotes de dados de uma aplicação são
roteados com base no sistema final de destino. No caso do GMTP, diferentemente
de qualquer outros protocolo existente, para o cenário ilustrado no exemplo
supracitado, quando qualquer nó \clis $\in$
...
\repassu{7} analisará o pedido pelo nome do fluxo de dados \setpks e decidirá
por encaminhar os pacotes de dados \pks $\in$ \setpks para o respectivo nó
\repasss solicitante, naquele caso o nó \repassu{11}. Isto só é possível porque
o nó \servs instrui o nó \repassu{7}, que é o primeiro nó comum entre os
caminhos \setwayiu{1} e \setwayiu{2}, a se tornar transmissor de \setpk, ou
seja, \transmitqu{\repassu{7}} $=1$. Como resultado, o nó \repassu{7} aciona
um filtro para interceptar todo pacote de requisição para obter o fluxo de dados
\setpk. A partir desse instante, como o nó \repassu{7} interceptará todos os
pedidos de conexão com interesse em obter \setpk, \repassu{7} responderá ao nó
\repasss solicitante como se fosse o nó \servu{1}. Salienta-se
também que há originalidade nesse algoritmo a partir do momento em que se
permite que um nó \servs se comunique com um nó \repasss a fim de delegar a
responsabilidade em distribuir um determinado fluxo de dados \setpk.
...
% suspende a transmissão do fluxo de dados \setpks em direção ao nó \repassu{4}.
...
\SetAlgoLined
\caption{matchPartnersByPathIntersection(\repass: PeerRelay, \pks $=$
\pac{GMTP-Register})}
...
Sendo assim, o procedimento de seleção de nós parceiros do GMTP, natualmente,
sempre encontra o melhor parceiro quando houver disponibilidade no caminho em
direção ao nó servidor; caso contrário, solicita-se apenas um único fluxo de
dados ao nó servidor que, a partir deste ponto, alimenta-se todo o caminho
entre o nó servidor e o nó repassador solicitante. A partir desse ponto, caso
transmitam-se outros pedidos de registros de participação para obter um fluxo de
dados e estes sejam transmitidos através de pelo menos uma parte de um caminho
já utilizado para transmitir o fluxo de dados de interesse, com certeza, o nó
que interceptará tal pedido sempre será o melhor parceiro para repassar o fluxo
de dados \setpk. Este procedimento, inclusive, elimina a necessidade de um nó
repassador ter que formar parcerias com outros nós menos privilegiados. Isto
não elimina a possibilidade de um nó repassador formar múltiplas parcerias,
pois, para isto, basta transmitir um pedido de registro de participação em
direção a um outro nó servidor que também esteja gerando o fluxo de dados de
interesse.
...
de dados sobre \setpk, consequentemente utilizando-se pouco recurso de memória e
processador de um roteador. Com essa estratégia, deve-se esperar uma grande
quantidade de nós \clis capazes de reproduzir um fluxo de dados \setpk,
evitando-se sobrecarregar a rede com demasiadas transmissões do mesmo fluxo de
dados \setpks e a redução do tempo de inicialização para reproduzir o fluxo de
dados \setpk.
