...
apresentados aqui são base para o entendimento de como se verifica a
autenticidade de um conjunto de pacotes de dados, que corresponde a um fluxo de
dados multimídia correspondente a um evento ao vivo, transmitido por um servidor
a um ou mais clientes GMTP. Com relação a certificação digital, está permitirá
que os nós clientes em uma transmissão GMTP possam confiar no conteúdo
multimídia recebido de outros nós parceiros, através da validação do conteúdo
transportado dentro de um pacote de dados a ser reproduzido, ou seja, se
tal conteúdo foi realmente emitido pelo nó servidor, aquele gerador do fluxo de
dados de interesse do nó cliente.

Especificamente, os conceitos apresentados a seguir são referenciados no
Capítulo~\ref{cap:mudccp}, Seção~\ref{sec:seguranca}.
...
assinatura digital é construída a partir da combinação dos algoritmos de
criptografia assimétrica e das funções de \textit{hash}.

Como ilustra-se na Figura~\ref{fig:digital-sign}, para que um usuário
\textit{A} envie uma mensagem \textit{m} assinada digitalmente ao usuário
\textit{B}, aplica-se uma função de \textit{hash} à mensagem original
\textit{m}, gerando \textit{m'} e, em seguida, cifra-se \textit{m'} utilizando a
chave privada do emissor, resultando na assinatura digital. A mensagem e a
assinatura digital são enviadas ao receptor através de um canal de comunicação.
Note que, o emissor assina \textit{m'} com a sua chave privada, representada por
$K^{-}_{A}$, ao passo que o receptor obtém \textit{m'} utilizando a chave
pública do emissor, representada por $K^{+}_{A}$.

\begin{figure}[ht]
 \centering
 \includegraphics[scale=0.35]{imagens/DigitalSign.pdf}
 \caption{Geração de Assinaturas Digitais.}
 \label{fig:digital-sign}
\end{figure}

Para comprovar uma assinatura digital é necessário realizar dois procedimentos:
\begin{itemize}

  \item calcular o valor de \textit{hash} da mensagem \textit{m} (gerando
\textit{m'}); e

  \item decifrar a assinatura com a chave pública do emissor (resultando em
\textit{m'}). Em seguida, deve-se concluir o seguinte:

  \begin{itemize}

    \item Se os valores de \textit{hash} forem iguais, a mensagem
\textit{m} está íntegra (não sofreu qualquer alteração) e portanto a assinatura
está correta, pois foi gerada pela chave privada corresponde à chave pública
utilizada na verificação; ou

    \item Se os valores de \textit{hash} forem diferentes, a assinatura está
incorreta e portanto houve alterações na mensagem \textit{m} ou na
assinatura de tal mensagem.
...
digitais para atribuir autenticidade e integridade aos documentos. Este fato
tornou a assinatura digital um instrumento válido, inclusive juridicamente.
