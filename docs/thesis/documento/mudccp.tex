...
compartilharem o mesmo fluxo de dados \setpk, entregando apenas um fluxo de
dados \setpks para serem distribuídos aos nós \clis $\in$
$($\subsetcli$($\wayu{1}$)$ $\cup$ \subsetcli$($\wayu{2}$))$ e, possíveis,
para outros nós \clis que estejam conectados aos nós \repasss $\in$ \setwayi,
tal que o caminho \setwayis é constituído como resultado da parceria entre
\wayu{1} e \wayu{2}.
...
estiverem recebendo o fluxo de dados \setpks diretamente do nó \serv, este pode
instruir que \wayu{1} e \wayu{2} se tornem parceiros e apenas um fluxo de
dados \setpks seja transmitido, em vez de dois. Para isso, o nó \servs instrui
...
Considerando esses aspectos, o nó \repasss  a
...
\caption{Requisição e resposta da lista de eventos de um distribuidor de conteúdo.}
\SetAlgoLined
\SetAlgorithmName{Listagem}{listagem}{.}
...
\begin{itemize}
 ...
 \item O formato da mídia (H.261 video, MPEG video, etc.); e

 \item O endereço e porta para obter a mídia (multicast).

\end{itemize}

 \item Informação para verificação de autenticidade de cada pacote \pks $\in$ \setpk.
...
protocolo de rede e o endereço do nó \repass;
  ...
  \item \textit{f}, o certificado digital emitido pelo nó \servs para verificação
de autenticidade dos pacotes \pks $\in$ \setpk (opcional). Este assunto
...
\bigspace f=x509:http://vid12.akamai.com/certs/cert.crt\tcc*[f]{ver Seção~\ref{sec:seguranca}}\\
...
No exemplo apresentado na Listagem~\ref{algo:sdp-mediadesc}, utiliza-se a
