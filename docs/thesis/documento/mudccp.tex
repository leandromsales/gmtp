...
e direto que um nó \repasss pode executar para obter um fluxo de dados \setpk.
...
passos executados por esta abordagem de formação de parceria, onde a linha
tracejada na cor vermelha representa o caminho \setwayiu{1}. No Passo 1,
ilustra-se um cenário de rede com 19 nós \repasss e dois nós \serv. Nesse
instante, não existe nenhuma parceria efetivada. No Passo 2, ilustra-se o pedido
de registro de participação do nó \repassu{9} ao nó \servu{2}, o que constitui o
caminho \setwayiu{1}, como explicado na
Seção~\ref{subsec:registro-participacao}. Quando houver um nó qualquer \repasss
interessado por um fluxo de dados \setpk, motivado por algum nó \clis $\in$
\subsetcli$($\repass$)$, qualquer nó \ways pode interceptar um pedido de conexão
transmitido para \servs, tal que \way,\repasss $\in$ \setwayis e
\transmitqu{\way} $=1$. Ao interceptar, o nó \ways enviar uma resposta a
\repasss, como se fosse o servidor \serv, uma vez que o nó \ways já está
repassando o fluxo de dados \setpks para seus nós \clis $\in$ \subsetcli(\ways).
Para este caso, até então, \setpks estaria sendo repassado apenas para os nós
\subsetcli$($\way$)$, mas com essa estratégia de formação de parceria intra
\setwayi, o mesmo fluxo de dados \setpks é ``estendido'' até \repass, e este
poderá ser ``estendido'' para mais outros nós \repasss que utilizem o caminho
\setwayis para alcançar \serv. Este método de formação de parceria não
exige manutenção de conexão entre os nós de um dado caminho \setwayis.

Tal estratégia é muito interessante porque pode reduzir o tempo de
estabelecimento de conexão e reduzir a quantidade de requisições para o nó
\servs que transmite um fluxo de dados \setpk. Isto acontece porque se
...
Além disso, se o nó \repassu{q} por algum motivo falhar, que é aquele que
interceptou o pedido de conexão de um nó \clis $\in$ \repassu{q} e sendo assim
parceiro do nó \repass, o nó \repasss pode formar parcerias com os próprios nós
\repasss $\in$ \setwayid, tal que \setwayid\space $=$
\invert{$\delta($\repassu{q+1}, \setwayi$)$}, através de uma outra rota de
rede. Por exemplo, observe a
Figura~\ref{fig:esquema-abstrato-formacao-parceria-intra-falha}. Se o nó
\repassu{6} falhar e o nó \repassu{14} também estiver repassando \setpk, uma
...
repassar o fluxo de dados \setpk, aumentando a possibilidade de parcerias
...
No exemplo supracitado, constituiu-se um novo caminho \setwayiu{3} através da
junção entre o sub-caminho \invert{$\delta($\repassu{6}, \setwayiu{1}$)$} e o
sub-caminho $\delta($\invert{\setwayiu{2}},\repassu{15}$)$. Isto permite
...
identificação de um ou mais nós comuns a dois caminhos \setwayi. Esse caso foi
parcialmente introduzido no final da seção anterior, quando apresentou-se um
cenário de falha de um nó \repass. Na
Figura~\ref{fig:esquema-abstrato-formacao-parceria-interseccao}, ilustra-se um
cenário similar em que este método se adequa para formação de novas parcerias.

Especificamente, a formação de parceria por intersecção ocorre quando um nó
\repasss roteia um registro de participação ou um pedido de requisição por um
fluxo de dados \setpk, através de um caminho \setwayis que já esteja
repassando \setpk. No caso do exemplo ilustrado na
...
\repassu{11}, o nó \servs descobrirá, por intersecção, dois caminhos que tem
em comum os nós \setwayiu{1} $\cap$ \setwayiu{2} = \{\repassu{5},
\repassu{6}, \repassu{7}\} (Passo 2). Tal descoberta pode ser útil para o
método de formação de parcerias por combinação de \setwayi, a ser detalhado
mais adiante.

Generalizando o exemplo anterior, o método de formação de parcerias por
interseção de \setwayis cobrirá o seguinte caso: após o primeiro nó \clis $\in$
$\sum^{12}_{\repassi=8}$\subsetcli$($\repass$)$ solicitar o fluxo de dados
\setpk, o nó \repassu{7} será o nó comum entre os caminhos \setwayiu{1} e
\setwayiu{2}, tal que \transmitqu{\repassu{7}} $=1$. Dessa forma, para as
próximas requisições a fim de obter o fluxo de dados \setpk, originadas por
$\forall$\clis $\in$ $\sum^{12}_{\repassi=8}$\subsetcli$($\repass$)$, motivará
...
\repasss correspondente, como se \repassu{7} fosse o nó \servu{2}. Aspectos
relacionados ao envio de dados nesse contexto serão discutidos na
Seção~\ref{sec:asptransrecep}.
...
O método de seleção de nós por meio da combinação de caminhos \setwayi,
considera um nó \textit{pivot} \servs e o conjunto \setways de caminhos
conhecidos, mas que não se interceptam (Passo 1 na
Figura~\ref{fig:esquema-abstrato-formacao-parceria-combinacao}). Trata-se do
método mais poderoso para formação de parcerias entre os apresentados
anteriormente, porém é o mais complexo, podendo também ser o mais lento dos
três.

Com o registro de participação de cada nó \repasss em \servs (Passo 2), o nó
\servs passa a conhecer um conjunto de caminhos \setwayis $\subset$ \setway tal
que existem nós \repasss repassando um determinado fluxo de dados \setpk. Sendo
assim, por centralizar o conjunto dos possíveis caminhos \setwayis, \servs pode
determinar candidados a parcerios \repassu{q} de um determinado nó \repass. Note
que este é o único caso em que o nó \servs participa efetivamente do processo
de formação de parcerias.

Na Figura~\ref{fig:esquema-abstrato-formacao-parceria-combinacao}, ilustra-se
um simples exemplo da formação de parceria por meio da combinação de dois
caminhos \setwayiu{1} e \setwayiu{3}.
...
\setwayis conhecidos e determina quais são os melhores parceiros \repasssu{q}
...
Por fim, note também que, como ilustrado nas
Figuras~\ref{fig:esquema-abstrato-formacao-parceria-intra},
\ref{fig:esquema-abstrato-formacao-parceria-interseccao} e
\ref{fig:esquema-abstrato-formacao-parceria-combinacao}, a título de
possibilidades, \servu{2} pode compartilhar o conhecimento sobre os possíveis
caminhos conhecidos para outros nós \serv, nesse caso para o nó \servu{1}.
