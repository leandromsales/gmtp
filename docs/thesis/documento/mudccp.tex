...
conectar os nós \clis $\in$ \setclis, \repasss $\in$ \setrepasss e \servs
$\in$ \setservs, bem como transmitir os pacotes de dados \pks $\in$ \setpks
através da rede de sobreposição constituída na Fase 1
(Seção~\ref{sec:asptransrecep}); e

  \item \textit{Socket de aplicação e controle de congestionamento em \net:}
permitir a comunicação entre as aplicações clientes e servidores por meio das
funções de distribuição definidos na Fase 2, bem como controlar a taxa de
transmissão dos fluxos de dados transmitidos através da rede de sobreposição
formada na Fase 1 (Seção~\ref{sec:ccgmtp}).
...
no momento do rotear tal pacote para o próximo salto da rota em direção ao nó
\serv. Na prática, o identificador de um nó repassador pode ser o endereço IP.
Quando o pacote \pac{GMTP-Register} alcançar o destino \serv, o nó \servs
conhecerá a lista ordenada dos nós \repasss até \servs e a armazenará como sendo
um dos possíveis caminhos para distribuir um fluxo de dados \setpk. Como
resposta ao nó \repass, o nó \servs deve enviar um pacote do tipo
\pac{GMTP-Register-Reply}, que confirma o registro de participação e informa
o caminho \setwayis que acabara de ser descoberto. O caminho \setwayis pode ser
utilizado futuramente no processo de formação de parcerias, a ser discutido na
...
parcerios \repassu{q} para futuras requisições. Neste caso, pode-se utilizar
\repasss para repassar pacotes de dados \pks mesmo quando \subsetcli$($\repass$)
= \{\emptyset\}$, ou seja, quando o nó \repasss não tenha nó clientes para
repassar o fluxo de dados \setpk. De forma similar, caso $\exists$\clis $\in$
\subsetcli$($\repass$)$ interessado em obter \setpk, com \transmitr \space$ =
1$, ou seja, quando um nó \repasss já está recebendo o fluxo de dados \setpk,
reduzi-se o tempo de início de reprodução do fluxo de dados \setpks para o nó
\clis em questão, pois basta que o nó \repasss comece a encaminhar o mesmo fluxo
de dados \setpks também para o nó \cli. Neste caso, o nó \repasss começa a
transmitir \setpks em modo multicast. Este assunto será retomado na
...
\repasss não é obrigado a informar qual fluxo de dados \setpks está
interessado, tal como explicado anteriormente. Porém, se um fluxo de dados
\setpk for especificado, o nó \servs executará uma função para determinar se
aceita o pedido de conexão para transmitir \setpks ou se delega essa
transmissão para algum nó repassador \repasssu{q} que se tornará parceiro do nó
requisitante \repass.
...
\SetKwFunction{Union}{Union}\SetKwFunction{respondToClient}{respondToClient}
\SetKwFunction{Union}{Union}\SetKwFunction{getChannelOfFlow}{getChannelOfFlow}
\SetKwFunction{Union}{Union}\SetKwFunction{GMTPRegister}{GMTP-Register}
...
\tcc{The node \repasss uses this function to send a register of participation to
...
\If(){\pks $\neq$ NULL} {
  ...
  \textit{\cli} \attrib \extractAttribute{\pk, `client'}\tcc*[r]{Extracts \cli in \pk}
  \uIf(){\isFlowBeingReceived{\setpk}} {
    \tcc{Let \clis know that \setpks is already registered in this \repass. The function \respondToClient is defined in section \ref{subsec:conexao-requisicao}.}
    \Return{\respondToClient{\setpk, \cli}}\;
  } \ElseIf() {\textbf{not} \isWaitingRegisterReply{\setpk}} {
      \tcc{send request to \servs and wait for \pac{GMTP-Register-Reply}. When received, executes onReceiveGMTPRegisterReply, defined in the Algorithm~\ref{algo:onReceiveGMTPRegisterReply}.}
      \waitRegisterReply(\setpk)\;
      \Return{\sendPktRdt{\GMTPRegister{\serv, \setpk}}}
  }
}
\If(){\textbf{not} \isWaitingRegisterReply{\serv}} {
  \tcc{Same as before, but just register participation.}
...
Quando o pacote do tipo \pac{GMTP-Register-Reply} chegar no nó \repass, que
solicitou o registro de participação, aciona-se o procedimento apresentado no Trecho de
Código~\ref{algo:onReceiveGMTPRegisterReply}, executando-se os passos
...
\If(\tcc*[f]{\servs confirmed registration}){\pks = OK} {
  \servs[\length{servs}] \attrib \extractAttribute{\pk, `serv'}\tcc*[r]{Gets \serv}
  \textit{\setpk} \attrib \extractAttribute{\pk, `flow'}\tcc*[r]{Gets \setpks in \pk}
  \If(){\setpk $\neq$ NULL}{
    \textit{port} \attrib \extractAttribute{\pk, `port'}\tcc*[r]{Gets \setpks in \pk}
    \textit{channel} \attrib \createMulticastChannel{\serv, port}\;
    \Return{\respondToClient{\textit{channel}}}\;
  }
  pkt \attrib \recvPktRdt{GMTP-Register-Reply}\;
  \uIf{pkt = OK} {
    servs[\length{servs}] \attrib \serv\;
    \setwayis \attrib \parsePath(pkt)\tcc*[r]{get the \setwayis until \serv} \nllabel{ln:parsepath}
    \If(){\clis $\neq$ NULL}{
...
\SetKwFunction{Union}{Union}\SetKwFunction{GMTPResponse}{GMTP-Response}
...
Implementar o algoritmo da função \respondToClient:
\respondToClient{\textit{channel}} % nessa tem que buscar os clientes esperando por resposta
\respondToClient{\setpk, \cli}
