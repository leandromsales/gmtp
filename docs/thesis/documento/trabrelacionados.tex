...
6121327,
4378429,
https://peercdn.com/ - só para conteúdos estáticos, mais uma para fragmentar
Is there a future for mesh-based live video streaming? (Set/2008) - analisar
pra ver se rola...
...
Alguns protocolos padronizados pela IETF foram especificados, implementados e
disponibilizado publicamente. Em geral, os protocolos dessa categoria permitem a
criação, encerramento e controle de conferências, como videoconferência e
uma transmissão ponto-a-ponto de um conteúdo de audio/vídeo em tempo real.
Nesta perspectiva, a seguir, apresentam-se os protocolos de rede que merecem
destaque, principalmente por serem empregado na maioria das aplicações
multimídia existentes.

\subsection{H.323 / ITU-T}
...
tempo real por \textit{multicast} já estivessem sendo realizadas no
Mbone~\cite{819167} há algum tempo, não havia ainda qualquer padrão aberto para
controle de conferências multimídia.

\subsection{SIP -- \textit{Session Initiation Protocol}}
...
\subsection{RTP -- \textit{Real Time Protocol}}
...
\subsection{RTSP -- \textit{Real Time Streaming Protocol}}
...
estabelecer e controlar um único ou vários fluxos sincronizados de mídias
contínuas pertencentes a uma apresentação.
...
HTTP ou outros meios, armazenado em um local diferente do servidor de mídia. Uma
descrição de apresentação pode conter informações sobre um ou mais fluxos que
compõe a apresentação, como endereços de rede e informações sobre o conteúdo da
apresentação, além de parâmetros que tornam possível ao cliente escolher a
combinação mais apropriada das mídias.

\subsection{ALTO -- \textit{Application-Layer Traffic Optimization}}
...
% http://ieeexplore.ieee.org/xpl/articleDetails.jsp?tp=&arnumber=5935134&queryText%3DPeerCast
...
% http://ieeexplore.ieee.org/xpl/articleDetails.jsp?tp=&arnumber=4146695&queryText%3DPULSE+p2p
...
Falar aqui que a quantidade de aplicação é tanta e estas podem ser clientes do
GMTP.
...
GridMedia~\cite{zhao_gridmedia+:_2009}, Sopcast~\cite{fallica_quality_2008},
AnySee~\cite{huang_anysee2:_2007}, PPLive~\cite{hei_insights_2006} e o
ZIGZAG~\cite{tran_zigzag:_2003}. Estas tecnologias têm sido utilizadas com
sucesso para a aplicações multimídia, contudo tais soluções foram concebidas
para distribuição de conteúdo multimídia considerando um determinado propósito,
com pouca flexibilidade no ponto de vista de extensibilidade e reúso dos
...
\subsection{CCNx -- \textit{Content Centric Network Protocol}}

Preencher...

\subsection{NDN -- \textit{Named Data Network}}
