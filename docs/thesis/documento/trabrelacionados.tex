...
trabalhos sobre aplicações e seus protocolos para distribuição de mídias ao
vivo. Nesse contexto, inclui-se também os trabalhos relevantes que, apesar de
não apresentarem uma proposta de protocolos de transporte/rede para distribuição
de mídias ao vivo, apresentam-se com alguma similaridade se comparado ao
\mudccp.
...
ao vivo. Em seguida, na Seção~\ref{sec:proto-trans-ea}, discute-se o estado da
arte acerca de protocolos de transporte para esse mesmo fim, dando-se ênfase às
propostas de protocolos que utilizam uma abordagem P2P e ou P2P/CDN. Na
Seção~\ref{sec:trab-redes-centradas-conteudo}, apresenta-se os proeminentes
trabalhos relacionados às redes centradas no conteúdo. Por fim, na
Seção~\ref{sec:trabrel-sumario-comp}, apresenta-se uma breve discussão
comparativa sobre os principais protocolos discutidos e o GMTP.
...
Adaptive
Streaming over HTTP}~\cite{6077864,5986186}, e o {Smooth Streaming}~\cite{REF},
%  e o WebRTC
...
baseado no protocolo HTTP. Adaptado de~\cite{Pantos2013}.}
...
arquitetura
P2P~\cite{4509752,susu_xie_coolstreaming:_2007,
...
Os autores do DONet/CoolStreaming~\cite{xinyan_zhang_coolstreaming/donet:_2005}
(\textit{Data-drive Overlay Network}), como foi nomeado, propuseram que partes
da mídia fossem distribuídas sobre uma rede de sobreposição onde os nós sempre
repassavam os dados para outros nós interessados pelo mesmo dado, sem nenhuma
regra pre-definida, como nó pai e nó filho; interno ou externo; capacidade de
upload e download, etc. Com essa visão centrada no dado, os autores decidiram
que a disponibilidade do dado era o critério para guiar as estratégias de
escalonamento dos fluxos de dados (quais nós deveriam transmitir e quais nós
deveriam receber os dados) transmitidos através da rede de sobreposição,
adaptando-se melhor às dinâmicas dos nós em uma rede P2P. Todos os
nós periodicamente trocavam informações sobre a sua disponibilidade de
\textit{chunk} com um conjunto de nós parceiros, e obtinham os dados não
disponíveis a partir dos nós os anunciavam como disponíveis.

O DONet/CoolStreaming se tornou uma solução proeminente para distribuição de
conteúdos multimídia em larga escala. Isto porque não é necessário construir uma
estrutura global e de manutenção complexa e à medida que as parcerias são
formadas e as informações sobre disponibilidade dos dados são trocadas entre os
nós participantes, permite-se uma mudança adaptativa entre os nós provedores de
dados.
...
servidor, às mais complexas, como as baseadas em P2P e/ou CDN. Diante desse
...
como apresentou ao longo dessa seção. Apesar desse filtro, pode-se afirmar que
as soluções propostas nessa área de pesquisa são apresentadas considerando os
aspectos discutidos no Capítulo~\ref{cap:fundamentacao}. Em geral, decide-se
sobre se a estrutura da rede de sobreposição será em árvore e/ou em malha; sobre
se os nós realização periodicamente requisições das partes da mídia
(\textit{pull-based}) e/ou se os nós transmissores enviarão as partes da mídia
para o nó receptor após este último sinalizar interesse por tal conteúdo
(\textit{push-based}); e, sobre a arquitetura do serviço, se será P2P e/ou CDN.

Nesse contexto, durante o levantamento bibliográfico realizado no
contexto deste trabalho, catalogou-se outras propostas que não foram detalhadas
nas seções anteriores, mas que devem ser mencionadas. São elas:
...
ZIGZAG~\cite{tran_zigzag:_2003}. Além dessas, diversas outras soluções foram
encontradas, mas foram excluídas porque não se apresentaram como trabalhos
acadêmicos, mas sim como produtos de software, não sendo possível referênciá-las
formalmente.
