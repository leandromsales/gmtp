...
% \linesnumbered
%
% \Entrada{Solução $s$, Inteiro $ILSmax$, Inteiro $kp_{0}$, Função $f$(.)}
% \Saida{Solução $s^{*}$ de qualidade superior ou igual à $s$ de acordo com a
% função $f$}
...
% {
% $iter \leftarrow 0$\;
% \Enqto{ $iter < ILSmax$ \textbf{and} critério de parada não satisfeito }
% {
% $s' \leftarrow$ perturbação($s^{*}, kp$)\;
% $s'' \leftarrow$ BuscaLocal($s', f$)\;
%
% \eSe{ $s''$ for melhor que $s^{*}$ de acordo com a função $f$ }{
% $s^{*} \leftarrow s''$\;
% $kp$ $\leftarrow$ $kp_{0}$\;
% $iter$ $\leftarrow$ $0$\;
% }{
...
% \renewcommand{\listalgorithmname}{Lista de Trechos de Código Fonte}

% \renewcommand*{\listalgorithmcfname}{Lista de Algoritmos}
% \renewcommand*{\algorithmcfname}{Algoritmo}
% \renewcommand*{\algorithmautorefname}{Trecho de Código}
...
% \Titulo{Um protocol \textit{Cross-Layer} para Distribuição de Mídias Ao Vivo
% pelo Compartilhamento de Fluxos entre Roteadores em Redes Híbridas CDN/P2P}
