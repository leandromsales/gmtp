...
$\in$ \subsetcli$($\repass$)$. Este passo se repete continuamente.
...
por último. Este passo se repete continuamente.

\end{itemize}

  \item Após obter a lista de candidatos a parceiros (Passo 1), o nó \repassu{3}
envia requisições do tipo \pac{GMTP-Request} em direção a outro nó \repassu{2}
que já esteja recebendo um fluxo de dados \setpk. Como ilustra-se na
...
recepção de fluxos de dados, tal como ilustra-se na
Figura~\ref{fig:tabela-recepcao-fluxo}. Nesse exemplo, nota-se que um nó
...
\repassu{q}. Por exemplo, dentre os fluxos de dados que o nó \repass está
...
transmissões para um mesmo fluxo de dados \setpks originado em \servs
...
soluções tradicionais de transmissão multicast, as quais os nós clientes, camada
de aplicação, tinham que se adaptar às configurações estáticas dos canais
multicast definidos pelo administrador de rede, e até os próprios
administradores de rede tinham que fazer tal configuração de forma manual, que
obrigatoriamente tinha que ser realizada em todos os nós roteadores de um
determinado caminho.
...
dos pacotes de dados \pks com a largura de banda de rede disponível;
...
que atual em transmissões unicast, chamado de \textit{\mudccps Unicast
...
$d_0$ ao valor de $RTT_{s}$. $RTT_{s}$ é o tempo de ida e volta calculado na nó
transmissor.
...
diferencia o GMTP-UCC do RCP e este assunto será retomado a seguir.
...
pacote \pks com o valor de $R_{p} = \infty$ em direção a \serv.
  ...
  \item Todo nó \way, ao receber um pacote \pk, verifica se $R(t) < R_{p}$, em
caso afirmativo, então $R_{p} \leftarrow R(t)$, caso contrário nenhuma
modificação é realizada nesse campo. Nesse ínterim, se \transmitqu{\way}
$=$ $1$, \ways também executa as seguintes ações:
...
móvel do RTT no intervalo de $d_0$ (Equação~\ref{eq:calcrtt-rcp}) e assim
utilizar para atualizar $R(t)$;
...
com a Equação~\ref{eq:cc-rcp}, no intervalo de tempo correspondente a $d_0$.

  \item Ao receber um pacote contendo $R_{p}$, o nó \servs ajusta sua taxa
de transmissão para o valor de $R_{p}$.
...
\setwayi em vários sub-caminhos \setwayia. Com isto, se existir largura de banda
disponível entre \transu{1} e \transu{2}, ou seja, $C - y(t) > 0$, então o
...
\transu{2}. Por este motivo, o tempo $d_0$ é definido entre dois nós \transs e
...
% RTT corresponde à $\frac{q(t)}{d_0}$~\cite{Dukkipati:2005:PSF:2103175.2103204}.
...
determinar o novo valor da taxa de transmissão do nó transmissor, que estará
disponível em $R_{p}$. Porém, essa estratégia pode limitar alguns nós de
receberem os pacotes de dados \pks $\in$ \setpks em uma taxa maior, quando
disponível. Por exemplo, observe o cenário ilustrado na
Figura~\ref{fig:ccgmtp-interacao-2}, abstraindo-se os nós clientes. Nesse
cenário, observa-se um caminho \setwayi $=$ $\{$\transu{1}, \wayu{2},
\transu{3}, \wayu{4}$\}$. Isto significa que existem nós \clis $\in$
$($\subsetcli(\transu{1}) $\cup$ \subsetcli(\transu{3})$)$ recebendo os
pacotes de dados \pk. Ao utilizar apenas o RCP, o nó \servs transmitirar
pacotes de dados \pks a uma taxa de transmissão de \ut{1}{Mb/s} tanto para os
nós \clis $\in$ \subsetcli(\transu{1}) quanto para os nós \clis $\in$
\subsetcli(\transu{3}). Ora, isso faz sentido apenas para os nós \clis $\in$
\subsetcli(\transu{1}) e não para os nós \clis $\in$ \subsetcli(\transu{3}),
visto que em \transu{3} o valor de $R(t)$ é igual a \ut{4}{Mb/s} e o nó
\wayu{4} não limita o uso dessa taxa de transmissão para os nós \clis $\in$
\subsetcli(\transu{3}), já que corresponde a \ut{8}{Mb/s}.
...
transmissão, porém isto pode limitar alguns nós clientes a receberem em uma
taxa maior.}
\label{fig:ccgmtp-interacao-2}
\end{figure}

No caso do GMTP-UCC, esta limitação foi superada ao determinar que se
\transmitqu{\way} $=$ $1$, ou seja, \way $=$ \transs $\in$ \settrans, então a
...
\transu{\transi+1}. Por isso, o GMTP-UCC segmenta o caminho \setwayi de acordo
com a menor taxa de transmissão entre dois nós \transs e \transu{\transi+1}.
Como resultado dessa estratégia adotada no GMTP-UCC, o cenário ilustrado na
Figura~\ref{fig:ccgmtp-interacao-2} pode ser atualizado e ilustrado na
Figura~\ref{fig:ccgmtp-interacao-3}.
...
transmissão de um fluxo de dados para certos nós clientes.}
...
\subsubsection{Diferença entre o GMTP-UCC e o RCP:}

- quem inicia é o cliente, não o servidor
- segmentação do caminho
- o cliente sabe a qual taxa os pacotes irão chegar (pode adaptar fluxo)
- sugestão de caminhos com base na capacidade de transmissão

\subsubsection{Escolha do algoritmo RCP em detrimento ao TCP e ao XCP:}

Essa seção foi escrita com base na
referência~\cite{Dukkipati:2005:PSF:2103175.2103204}, incluindo seus gráficos,
que foram extraídos da mesma referência.
...
computação por cada pacote no roteador. Tanto o RCP quanto o XCP~\cite{REF} são
os protocolos mais conhecidos do estado da arte que tentam emular um PS e, por
este motivo, suas equações de controle de congestionamento são similares. Porém,
o modo que o RCP e o XCP tentam convergir suas respectivas taxas de transmissão
$R_{rcp}(t)$ e $R_{xcp}(t)$ é bastante diferente, alocando-se tais taxas para
cada fluxo de dados a fim de emular a taxa de transmissão do PS, definida por
$R_{ps}(t)$. Dessa forma, tornou-se fundamental decidir qual dos dois protocolos
seria mais adequado ao GMTP-UCC e, para tomar tal decisão, estudou-se as
diferenças entre tais protocolos, com base no que se apresenta a seguir.
...
valores de PS foram calculados a partir de expressões analíticas.}
...
trânsito na rede em qualquer instante.
...
Este procedimento ocorre em um intervalo de tempo definido por $d_0$ (vide
...
complexidade é menor e há uma redução significativa de convergência entre a
...
permite-se que fluxos de dados GMTP/RCP e TCP/RCP coexistam na Internet de
