...
\item Na Seção~\ref{subsec:ccgmtp}, apresentam-se detalhes de funcionamento dos
...
desconexão, eleição de nós relatores e segurança.
...
\subsection{Principais funções do GMTP}
...
\setcounter{enumi}{-1}
  ...
  \item \textit{GMTP-Request:} Cliente envia rquisição para obter um fluxo
...
parceria com outros nó repassador, dado um determinado fluxo de dados;
...
determinada transmissão;
  ...
  \item \textit{GMTP-DataPullRequest:} Repassador envia um pedido para obter o
mapa de buffer atual de um outro repassador parceiros;

  \item \textit{GMTP-DataPullResponse:} Resposta ao pedido para obtenção de um
mapa de buffer;

  \item \textit{GMTP-Elect:} Repassador envia para um cliente o pedido
para tal cliente atuar como nó relator;

  \item \textit{GMTP-ElectReply:} Cliente envia para o repassador uma
confirmação de que pode atuar como relator;

  \item \textit{Reservado:} Reservado para uso futuro e ignorado pelos nós que
o processa;

  \item \textit{Reservado:} Reservado para uso futuro e ignorado pelos nós que
o processa;

  \item \textit{GMTP-Close:} Servidor, repassador ou cliente solicita o término
de uma conexão;

  \item \textit{GMTP-Reset:} Determina, incondicionalmente, a finalização de
uma conexão.
...
e as conjecturas consideradas para a execução de
tal protocolo. Nas próximas seções, descrevem-se as funcionalidades do GMTP com
base em $\eta$, detalhando-se aspectos teóricos e computacionais empregados no
protocolo para a distribuição de um evento ao vivo \event. Para isto, segue-se
uma abordagem \textit{top-down}, partindo-se do processo de formação de $\eta$
e detalhando-se as suas funções dos diferentes tipos de nós a fim de
efetivamente realizar a distribuição de conteúdos multimídia.
...
\servs$\in$ \setserv. Isto pode ocorrer de forma direta ou indiretamente por
meio de outros repassadores \repassu{q} $\in$ \setrepass. Todo esforço realizado
nesse processo é visando otimizar a transmissão de um fluxo de dados \pks $\in
P$ a um ou mais nós \clis $\in$ \setcli, podendo ser distribuído pelos nós
\repasss por meio de diferentes caminhos \setwayis $\in$ \setway.

É importante compreender que o GMTP tenta determinar um caminho sub-ótimo
\setwayiu{\theta} através do qual os pacotes \pks sejam entregues mais
rapidamente quanto possíveis a \cli, resultando em uma melhor experiência em
assistir a um evento \event. Sendo assim, deve-se determinar o melhor caminho,
definido por \setwayiu{\theta}, tal que
\setwayiu{\theta} $= min(\zeta(\forall$\setwayi$))$ e, sempre que possível, que
\setwayiu{\theta} seja um caminho completo \setwayifu{\theta}. Sempre buscar um
caminho completo é importante porque como todos os nós são repassadores,
consegue-se distribuir o fluxo de dados \setpk para mais nós \cli. Além disso,
por possuir mais nós \repass, um caminho completo é mais tolerável à
desconexões. Esses e outros aspectos sobre a propriedade de um caminho completo
será discutir mais adiante.
...
é o primeiro passo, e um dos mais importantes. Este procedimento permite que um
nó \repasss se registre a um nó \servs para sinalizar interesse é funcionar
como repassador de um ou mais fluxos de dados \setpk.

Um nó \repasss envia uma mensagem de registro de participação a um nó \servs
utilizando o pacote \pac{GMTP-Register} e este envio permitirá a descoberta de
um caminho \setwayi. Isto porque todos os nós \repasss no caminho entre
\repasss e \servs devem alterar o pacote \pac{GMTP-Register}, adicionando seu
identificador que, na prática, pode ser o endereço IP. Quando o pacote
\pac{GMTP-Register} alcançar o destino \serv, \servs terá a lista ordenada
dos nós \repasss até \servs e o guardará como sendo um dos possíveis caminhos
para distribuir fluxos de dados \setpk. Como resposta para \repass, o nó \servs
deve enviar um pacote do tipo \pac{GMTP-Register-Reply}, confirmando o registro
e informando \setwayis que acabara de ser descoberto. Posteriormente, \setwayis
será utilizado no processo de formação de parcerias, a ser discutido mais
adiante na Seção~\ref{sec:descparc}.

Como pode-se facilmente concluir, o registro de participação ocorre quando um nó
deseja participar da rede de sobreposição, não necessariamente quando se deseja
obter um fluxo de dados multimídia para assistir a um evento \event. Em ambos os
casos, o algoritmo de registro de participação é o mesmo. A diferença primordial
é que se um \repasss solicitar previamente um registro de participação a um
\serv, inicialmente sem interesse por um evento \events qualquer, será possível
mapeá-lo mais rapidamente para seleção de um subconjunto de possíveis nós
parcerios \repassu{q}. Neste caso, pode-se utilizar \repasss para repassar
pacotes de dados \pks mesmo quando \subsetcli$($\repass$) = \{\emptyset\}$. De
forma similar, caso $\exists$\clis $\in$ \subsetcli$($\repass$)$ interessado em
obter \setpks de um evento qualquer \event, com \transmitr \space$ =1$, pode-se
reduzir o tempo de início de reprodução do fluxo de dados \setpk, bastando para
isto \repasss começar a encaminhar o fluxo de dados \setpks para o nó \cli, o
que ocorre em modo multicast. Este assunto será retomado na
Seção~\ref{sec:asptransrecep}.

No Trecho de Código \ref{algo:registerRelay}, apresenta-se o pseudo-algoritmo
utilizado por um nó \repasss para se registrar a um nó \serv. Note que o
processo de registro não requer que um nó \repasss registre qualquer
interesse sobre um evento \event. Além disso, no final do procedimento do
registro de participação, o nó \repasss passa a conhecer sobre \setwayis através
do pacote \pac{GMTP-Register-Reply} (linha \ref{ln:parsepath}). Note também que
no GMTP toda transferência de pacotes de controle é feita com garantia de
entrega, com os nomes das funções representadas com o sufixo \_rdt
(\textit{reliable data transfer}).

Uma outra decisão importante tomada no GMTP é que um nó \repasss deve
periodicamente sinalizar sua participação na rede de favores $\eta$
(\textit{keep-alive}). Após o registro de participação, o nó \repasss
deve enviar periodicamente sinalizações de controle (\textit{polling}) sobre sua
participação na rede de favores $\eta$. Este procedimento deve ser feito usando
o pacote do tipo \pac{GMTP-Ack} em um tempo $t = max(300, t_{user}) - RTT$,
onde $t$ e $RTT$ são definidos em segundos, e $t_{user}$ é um parâmetro de tempo
definido pelo usuário. Quando \servs receber um pacote do tipo \pac{GMTP-Ack} do
nó \repass, este deve enviar um pacote do mesmo tipo. Caso \repasss não receba
...
\pac{GMTP-Reset}. Por outro lado, se \servs não receba um pacote do tipo
\pac{GMTP-Ack} no tempo de $t$, \servs deve enviar um pacote do tipo
\pac{GMTP-Ack} por 3 vezes a cada $4 \times RTT$. Um nó \repasss também pode
sinalizar explicitamente sua desconexão a \serv, quando não desejar mais
participar da rede de favores $\eta$. Para isto, deve-se enviar um pacote do
tipo \pac{GMTP-Close}. Em qualquer um dos casos apresentados de desconexão, por
expiração do tempo ou explícita, o nó \servs deve desconsiderar \repasss no
processo de formação de parcerias e enviar para o nó \repasss um pacote do tipo
\pac{GMTP-Reset}.\\
...
\SetKwFunction{Union}{Union}\SetKwFunction{recvPktRdt}{recvPkt{\_}rdt}
\SetKwFunction{Union}{Union}\SetKwFunction{sendPktRdt}{sendPkt{\_}rdt}
...
\If(){pkt = OK}{
  ...
  pkt \attrib \recvPktRdt{GMTP-Register-Reply}\;
  \uIf{pkt = OK}{
...
\serv} \nllabel{ln:parsepath}
    \If(){\clis $\neq$ NULL}{
      ...
      pkt $ \leftarrow$ \makePkt{GMTP-RequestNotify(1), \cli}\;
      \sendPktRdt{pkt}\;
    }
  }
  \Else{\tcc{Notify admin or try again?}
    \If(){\clis $\neq$ NULL}{
...
caminhos \setwayis serão conhecidos. Consequentemente, quanto mais caminhos
forem conhecidos, mais parcerias poderão ser formadas entre os nós \repass.
Portanto, quanto mais parcerias formadas, maior será o número de nós \clis
capazes de receber um fluxo de dados \setpks originado em \serv,
disponibilizado indiretamente através dos seus respectivos nós \repass, sem
nenhuma influência da camada de aplicação.

\subsection{Formação de parcerias entre nós \repass}
\label{sec:descparc}

A formação de parcerias entre dois ou mais nós \repass, consiste na ação de
selecionar nós \repassu{q} para um nó \repasss que possa auxiliar \repasss a
obter os pacotes \pks $\in$ \setpks
(Figura~\ref{fig:bucket-brigade-principle-5}). A formação de parcerias resulta
na constituição de um ou mais caminhos \setwayis $\in$ \setway, que interligará
um nó \servs a um ou mais nós \clis $\in$ \subsetcli(\repass).
...
transmissão de um fluxo de dados \setpk, através dos métodos de formação de
parcerias apresentados a seguir, executa concorrentemente. A seguir, detalham-se
tais métodos.
...
\subsubsection{Formação de parcerias intra \setwayi:}
\label{subsec:parcintra}

Na formação de parceria intra \setwayi, os nós \ways $\in$ \setwayis são
automaticamente considerados parceiros entre si e por isto qualquer nó \ways
pode repassar um fluxo de dados \setpks para o nó \repasss em questão. A
formação da parceria ocorre de forma transparente durante o pedido de conexão de
um nó \clis ao nó \servs para obter \setpk. Dessa forma, qualquer nó \ways $\in$
\setwayis pode agir como se fosse um nó \serv, sendo este o método mais simples
e direto que um nó \repasss pode obter um fluxo de dados \setpks através de nós
parceiros \repassu{q}.
...
passos executados por este método de formação de parceria. No Passo 1,
...
efetivada. No Passo 2, ilustra-se a transmissão \transmissions do fluxo de dados
\setpk, que ocorre entre os nós \servu{2} e \repassu{9}, caracterizando o
...
Para formar o caminho \setwayiu{1}, o nó \repassu{9} transmitiu o pedido de
registro de participação ao nó \servu{2}, como explicado na
Seção~\ref{subsec:registro-participacao}. Assim, quando houver um nó
qualquer \repasss interessado por um fluxo de dados \setpk, motivado por algum
nó \clis $\in$ \subsetcli$($\repass$)$, qualquer nó \ways pode interceptar um
pedido de conexão transmitido para \serv, desde que \way,\repasss $\in$
...
do momento em que \ways interceptou o pedido de conexão enviado por \repass, o
 fluxo de dados \setpks estava sendo repassado apenas para os nós \clis $\in$
\subsetcli$($\way$)$. Porém, com o emprego do método de formação de parcerias
intra \setwayi, o mesmo fluxo de dados \setpks é ``estendido'' até \repass, e
tal nó poderá ``estender'' o fluxo de dados \setpks para mais outros nós
\repasss que utilizem o caminho \setwayis para alcançar \serv. Este método de
formação de parceria não exige manutenção de conexão entre os nós \ways $\in$
\setwayi, portanto não se consome recurso de rede para transmitir dados de
controle para o nó \serv. Note que quando um nó \repasss interceptar um pedido
de conexão para um fluxo de dados \setpk, tal como explicado anteriormente, o nó
\repasss deve também transmitir para o nó \servs uma notificação que informa
sobre tal pedido.

Esse método de formação de parceria é muito interessante porque pode reduzir o
tempo de estabelecimento de conexão e reduzir a quantidade de requisições para o
nó \servs que transmite um fluxo de dados \setpk. Isto acontece porque se
permite que \repasss descubra mais rapidamente outros candidatos a parceiros
\repassu{q}, tal que \transmitru{q} $ = 1$, efetivando-se parcerias quando
ocorrer \subsetcli$($\repass$)\neq \{\emptyset\}$.
...
de constituição de um novo caminho \setwayiu{3} formado pelo método de formação
de parceria intra \setwayi.}
\label{fig:esquema-abstrato-formacao-parceria-intra-falha}
\end{figure}

Uma ação específica é realizada se um nó \repassu{q} falhar por algum motivo,
sendo \repassu{q} aquele nó que tenha interceptado o pedido de conexão de um nó
\repass, formando-se uma parceria entre eles. Para tratar estes
casos, definiu-se que o nó \repasss pode formar parcerias com os próprios nós
\repasss $\in$ \setwayid, tal que \setwayid\space $=$
\invert{$\delta($\repassu{q+1}, \setwayi$)$}, através de uma outra rota de
rede que também alcance o nó \servs -- isto pode acontecer devido à
execução de algoritmos de roteamento dinâmico \textit{Intra-AS (Autonomous
Systems)}, por exemplo, o OSPF, e \textit{Inter-AS}, por exemplo, o
BGP~\cite{kurose2006}.
...
\repassu{14}. Isto porque o nó \repassu{14} interceptará o pedido de conexão do
nó \repassu{7}. Se nenhum dos dois nós \repassu{14} e \repassu{13} estiverem
repassando \setpk, o pedido de conexão enviado pelo nó \repassu{9} alcançará o
nó \servu{2}, que estabelecerá uma nova conexão. Isto fará com que todo o
caminho entre \repassu{9} e \servu{2}, passando por \repassu{14}, comece a
rotear o fluxo de dados \setpk, aumentando a possibilidade de parcerias
futuras com nós \repasss cujo pedido de conexão para obter \setpks seja roteado
por algum nó do novo caminho. O método de formação de parceria por intersecção
cobrirá estes casos, sendo tal método detalhado mais adiante.

No exemplo supracitado, constituiu-se um novo caminho \setwayiu{3} $=$
\setwayidu{1} $\cup$ \setwayidu{2}, tal que \setwayidu{1} $=$
\invert{$\delta($\repassu{6}, \setwayiu{1}$)$} e \setwayidu{2} $=$
$\delta($\invert{\setwayiu{2}}, \repassu{15}$)$. Como resultado, permite-se
reduzir o tempo de (re)início de reprodução do evento \events por um nó \cli. Na
Seção~\ref{subsec:desconexao}, apresenta-se uma discussão mais geral sobre o
comportamento do GMTP em outros casos de desconexões. O método de formação de
parceria intra \setwayi, apresentado nesta seção, está intimamente relacionado
com o processo de estabelecimento de conexão do GMTP, detalhado mais adiante na
Seção~\ref{subsec:conexao-requisicao}.

\subsubsection{Formação de parcerias por intersecção de \setwayi:}
\label{subsec:parcintersec}

O processo de formação de parcerias por intersecção consiste na identificação de
um ou mais nós comuns a dois caminhos \setwayi. Esse caso foi parcialmente
introduzido no final da seção anterior, quando apresentou-se um cenário de falha
de um nó \repass. Nesta seção, apresenta-se um cenário similar em que este
método se adequa, ilustrado na
...
Especificamente, a formação de parceria por intersecção ocorre quando um nó
\repasss transmite um pedido de registro de participação ou um pedido de
requisição por um fluxo de dados \setpk, através de um caminho \setwayis que já
esteja repassando \setpk. No caso do exemplo ilustrado na
Figura~\ref{fig:esquema-abstrato-formacao-parceria-interseccao}, após o registro
de participação de \repassu{9} e a consequente constituição de
\setwayiu{1} (Passo 1), se qualquer um dos nós \repassu{10..12}, por exemplo
\repassu{11}, solicitar o registro de participação ou transmitir um
pedido de conexão para obter \setpks (Passo 2 da
...
obter \setpk. Note que pelo registro de participação de \repassu{9} e
\repassu{11}, o nó \servs descobrirá, por intersecção, um conjunto de nós
\repasss $\in$ $($\setwayiu{1} $\cap$ \setwayiu{2}$)$ (Passo 2).

Generalizando, quando um nó \servs receber um pedido de conexão para
transmitir um fluxo de dados \setpk, este executa um algoritmo interativo
comparando os caminhos conhecidos \setways com o caminho \setwayis contido no
pedido de conexão que acabara de ser recebido. No Trecho de
Código~\ref{algo:findPartnerIntersectPath}, apresenta-se um pseudo-algoritmo
para este caso.

No caso do cenário ilustrado na
Figura~\ref{fig:esquema-abstrato-formacao-parceria-interseccao}, identifica-se
um sub-caminho \setwayidu{1} $=$ \setwayidu{2} = \invert{$\delta($\repassu{7},
\setwayiu{1}$)$} $\cup$ \invert{$\delta($\repassu{7}, \setwayiu{2}$)$} =
$\{$\repassu{7}, \repassu{6}, \repassu{5}, \servu{2}$\}$. Ao identificar tal
intersecção, o nó \servu{2} deve enviar apenas um fluxo de dados \setpk,
unicast, para \repassu{7}, que o transmitirá tantas vezes forem as requisições
de pedido de conexão para obter \setpk, originadas em qualquer nó \repasss $\in$
$\delta($\repassu{7}, \setwayi$)$, $\forall$\setwayi. Este mecanismo, original
do GMTP, permite repassar pacotes de dados \pks levando em consideração o
conteúdo de interesse \textit{(content centric networks)}, e não o endereço de
destino \textit{(host centric networks)}. Isto porque, a partir do uso do GMTP,
quando qualquer nó \clis $\in$ $\sum^{12}_{\repassi=8}$\subsetcli$($\repass$)$
solicitar o fluxo de dados \setpk, o nó \repassu{7} será o nó comum entre os
caminhos \setwayiu{1} e \setwayiu{2}, tal que \transmitqu{\repassu{7}} $=1$.
Dessa forma, para as próximas requisições a fim de obter o fluxo de dados
\setpk, originadas por $\forall$\clis $\in$
$\sum^{12}_{\repassi=8}$\subsetcli$($\repass$)$, motivará
algum nó \repassu{8..12} a enviar tal requisição para \servu{2}, que
passará obrigatoriamente por \repassu{7}. Neste caso, o nó \repassu{7}
interceptará o pedido de conexão para obter \setpks e responderá para o nó
\repasss correspondente, como se \repassu{7} fosse o nó \servu{2}.

% Para generalizar, apresenta-se no Trecho de
% Código~\ref{algo:findPartnerIntersectPath}, um pseudo-algoritmo para a formação
% de parceria por intersecção, executado em um nó \serv.

 - REVER ESSE ALGORITMO PARA COLOCAR A DEFINIÇÃO DO CUSTO E COBRIR O CASO EM QUE
UMA PARCERIA É FORMADA DIRETAMENTE ENTRE DOIS NÓS REPASS, SE R4 ESTIVER
RECEBENDO FLUXO DE DADOS DE S1 E R9 QUISER PEGAR FLUXO DE DADOS EM S2, R9 PODE
OBTER O FLUXO DE DADOS VINDO DE S1 PARA R4, BASTA S1 E S2 TROCAREM FIGURINHAS
SOBRE OS CAMINHOS DE W QUE AMBOS CONHECEM. FAZ SENTIDO?
 - REVER O LOOP DO ALGORITMO.
 - ESSE ALGORITMO DEVE EXPRESSAR O SEGUINTE: DADOS CAMINHOS SIMILARES ACIMA DE
mspf QUE CONTENHAM NÓS RECEBENDO P, ENVIAR PARA UM NÓ R QUE ACABARA DE PEDIR
CONEXÃO A LISTA DE POSSÍVEIS PARCEIROS
...
\subsubsection{Formação de parcerias por combinação de \setwayi:}
\label{subsec:parccombina}

O método de formação de parcerias por combinação, como o próprio nome o define,
combina dois (ou mais) caminhos distintos \setwayiu{1} e \setwayiu{2} tal que
\setwayiu{1} $\cap$ \setwayiu{2} $=$ \{\serv\}, ou seja, os caminhos se
interceptam apenas no nó \servs, como ilustra-se no Passo 1 da
Figura~\ref{fig:esquema-abstrato-formacao-parceria-combinacao}). Dessa forma,
este método considera um nó \textit{pivot} \servs e o conjunto \setways de
caminhos conhecidos para realizar as combinações. Por isso, faz-se necessário
que o algoritmo que determina tais combinações seja executado em \serv, que
instrui os nós a serem combinados a efetivar a parceria. Trata-se de um
algoritmo mais complexo se comparado com os outros dois apresentados
anteriormente, mas pode ajudar sobremaneira os nós \repasss a expandirem suas
possibilidades de parcerias, incluindo obtendo pacotes de dados \pks $\in$
\setpks de múltiplas fontes.

Com o registro de participação de cada nó \repasss em \serv, como
ilustra-se no Passo 2 da
Figura~\ref{fig:esquema-abstrato-formacao-parceria-combinacao}, o nó
\servs passa a conhecer um conjunto de caminhos \setwayis $\subset$ \setways tal
que existem nós \repasss repassando um determinado fluxo de dados \setpk. Sendo
assim, \servs pode determinar candidados a parcerios \repassu{q} de um
determinado nó \repass, pois tal nó centraliza o conjunto dos possíveis caminhos
\setwayi. Este é o único caso em que o nó \servs participa efetivamente
do processo de formação de parcerias.
...
Em geral, o algoritmo funciona da seguinte forma: \servu{2} processa os caminhos
...
haverá duas opções de combinação, uma ilustrada no Passos 3 e outra ilustrada
no Passo 4 da Figura~\ref{fig:esquema-abstrato-formacao-parceria-combinacao}.
Para decidir qual opção escolher, o nó \servu{2} compara os custos
$\zeta($\setwayiu{1}$)$ e $\zeta($\setwayiu{2}$)$. Se $\zeta($\setwayiu{1}$)
\geq \zeta($\setwayiu{2}$)$, então o nó \servu{2} solicita que \repassu{9}
envie um pedido de conexão para \repassu{16} (Passo 4). Esta é a grande
motivação da formação de parceria por combinação, pois quando o pedido de
conexão do nó \repassu{9} alcançar o nó \repassu{15}, este o interceptará e,
nesse instante, constitui-se um novo caminho \setwayis $=
\{$\repassu{15}$,$\repassu{14}$,$\repassu{6}$,$\repassu{7}$,$\repassu{8}$,
$\repassu{9}$\}$. Por fim, o nó \repassu{15} deve enviar uma notificação para o
nó \servu{2} informando sobre a constituição do novo caminho \setwayi. Em caso
de $\zeta($\setwayiu{1}$) \leq \zeta($\setwayiu{2}$)$, então o nó \servu{2}
...
instruirá o nó \repassu{6} a não interceptar tal pedido de conexão. Para a
\textit{flag} de finalização dá-se o nome de \textit{ignorar pedido de conexão},
...
de \setpk, envia um pacote do tipo \pac{GMTP-RequestNotify} para notificar
...
do uso do pacote do tipo \pac{GMTP-DataPullRequest}, que é preenchido com o mapa
...
\pac{GMTP-DataPullResponse}, que contém o mapa de buffer de quais
...
o procedimento de desconexão, adaptação de fluxo, eleição de nós relatores e
segurança.
...
\pac{GMTP-Elect} e aguardar por um \pac{GMTP-ElectReply}. Este procedimento
...
mal-intencionados poluam o sistema com conteúdos não originais, ou
seja, gerados pelo servidor (Figura~\ref{fig:bucket-brigade-principle-4}.
...
No GMTP, delega-se a questão de validação de um fluxo de dados \setpks para a
aplicação em execução no nó \cli que, opcionamente, pode verificar a
autenticidade dos pacotes de dados \pks. Isto pode ser feito através de
mecanismos de segurança já consolidados, como o de certificação digital. A
obtenção do certificado digital por um nó \clis deve ocorrer em modo unicast
entre este e o nó \repass, com garantia de entrega.

Apesar da verificação de autenticidade ser verificada pelo nó \cli, o nó
\repasss pode obter o certificado digital disponível no servidor. Isto é feito
através do download do certificado digital pela URL especificada no parâmetro
\textit{f} da descrição da mídia, como ilustra o no Trecho de
Código~\ref{algo:sdp-mediadesc}, Linha 8. Após o download do referido
certificado, o nó \repasss pode realizar cache de tal conteúdo para que os
próximos nós \clis possam obtê-lo mais rapidamente. Independente da forma do
download do certificado digital, uma vez o obtido, o nó \clis pode validar se o
conteúdo de dados de cada pacote de dados \pks $\in$ \setpks.

Como parâmetros de configuração, o administrador do nó \repasss pode habilitar
ou não a opção de obter automaticamente o certificado digital ou se prefere que
o nó \repasss o obtenha sob demanda. Para essa segunda opção, quando o primeiro
nó \clis desejar obtê-lo, a requisição para seu download é transmitida para a
URL especificada. É opcional também para o administrador escolher se o nó
\repasss deve realizar cache dos certificados digitais. Se a opção para
realização de cache estiver marcada, o nó \repasss deve obter o certificado
digital e atualizar o parâmetro \textit{f} da descrição da mídia para a nova URL
apropriada.
