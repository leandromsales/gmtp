...
Quando o pacote do tipo \pac{GMTP-Register-Reply} for recebido pelo nó \repass, o qual
solicitou o registro de participação, aciona-se o procedimento apresentado no Trecho de
Código~\ref{algo:onReceiveGMTPRegisterReply}, executando-se o passos restantes explicados
anteriormente.
...
\SetKwFunction{Union}{Union}\SetKwFunction{getServerPublicKey}{getServerPublicKey}
...
\uIf(\tcc*[f]{\servs confirmed registration}){\pks = OK} {
  ...
  \If(){\setpk $\neq$ NULL}{
    ...
    \servs[\length{servs}] \attrib \serv\tcc*[r]{Store \servs as connected.}
    \getServerPublicKey{\serv}\tcc*[r]{See Section~\ref{subsec:obterchavepublica}}
...
$= \{$\serv$\}$, ou seja, os caminhos que se interceptam apenas em \serv, como
...
$\cup$ \setwayiu{2}$)$ posicionados estrategicamente na rede a se tornarem
parcerios entre si a fim de compartilharem o mesmo fluxo de dados \setpks
otimizar o processo de transmissão (entregar o fluxo de dados mais rapidamente).
Tal procedimento pode ajudar sobremaneira os nós \repasss a expandirem suas
possibilidades de parcerias, incluindo a possibilidade de obter pacotes de dados
\pks $\in$ \setpks de múltiplas fontes \repassu{q}.

Com o registro de participação de cada nó \repasss em \serv, o nó \servs detém
o conhecimento dos caminhos \setwayis $\subset$ \setways através dos quais se
transmite o fluxos de dados \setpks e, por conseguinte, o nó \servs passa a
conhece os dois nós \wayu{\wayi_{1}},\wayu{\wayi_{2}} que estão recebendo o
mesmo fluxo de dados \setpk.

propõe-se que

 nós \repasss repassando um
determinado fluxo de dados \setpk. Sendo
...
mal-intencionados \repass poluam o sistema com conteúdos que não foram
...
$\in$ \setwayi verificam se o conteúdo de um pacotes de dados \pks $\in$ \setpks
foi alterado por algum nó \ways anterior, durante o procedimento de repasse.
Apenas após comprovar a autenticidade de um pacote \pk, o nó \ways repassa tal
pacote de dados \pks para o próximo nó \wayu{\wayconst+1}, transmitindo-os
também para seus nós \clis $\in$ \subsetcli$($\way$)$, se houver demanda. Este
procedimento evita que todos os nós \clis que receberem o fluxo de dados \setpks
tenham que verificar a autenticidade dos pacotes \pk.
...
de um nó \repasss $\in$ \setwayids alterar o conteúdo de um ou mais \pks
...
Sendo assim, quanto mais distante um nó \ways estiver da fonte geradora \servs
do fluxo de dados \setpk, mais pacotes \pks $\in$ \setpks deve-se verificar sua
...
such as the well-know md5 function. \servs get the value of data field, which
is the content that application wants to transport and generates a signature by
encrypt the hash of the data using the \servs private key. After, put the
generated signature in the signature field of the packet \pk. The signature
field will be used later by a note \repasss to verify the packet \pks
...
\textit{signatureHash} \attrib \hash{\textit{data}}\;
\textit{signature} \attrib encrypt($K^{+}_{\servconst_{\servi}}$, \textit{signatureHash})\;
\setPacketFieldValue{\pk, `data', \textit{signature}}\;
\Return{\pk}\;
...
isto, gera-se o código \textit{hash} do campo de dados e compara-se com o
código de \textit{hash} gerado pelo nó \servs no momento da transmissão do
pacote \pk. Note que o valor para o código de \textit{hash} gerado pelo nó
\servs é obtido através de processo de decriptar a assinatura do pacote de
dados \pks utilizando a chave pública do nó \serv. Assim, se o valor do
código de \textit{hash} gerado com base no conteúdo transportado no pacote \pks
for igual ao valor do código de \textit{hash} disponível na assinatura do
pacote, conclui-se que o pacote \pks não foi alterado por nenhum nó \ways $\in$
\setwayids $=$ \invert{$\delta(\wayconst_{\wayi+1}$, \setwayi$)$}, então marca-o
como aprovado para ser repassado, caso contrário, marca-o como desaprovado e
deve ser descartado. No Trecho de Código~\ref{algo:verifyPacketAuthenticity},
apresenta-se o procedimento de verificação de autenticidade de um pacote \pks
$\in$ \setpk.
...
\pks $\in$ \setpk' was modified. It marks those packets that were not modified
and those that were modified. For those packets not modified, the node \ways
make them ready for relaying, for those packets modified, it must be discarded.}
...
\ForEach{\pks $\in$ \setpk} {
  ...
  \textit{data} \attrib \getPacketFieldValue{\pk, `data'}\;
  \tcc{If next condition is True, packet \pks is market to be relayed,
otherwise \pks was modified by a node in \setwayids $=$
\invert{$\delta(\wayconst_{\wayi+1}$, \setwayi$)$} and must be destroyed.}
...
Um nó \repasss obtem a chave pública $K^{+}_{\servconst_{\servi}}$ de \serv
através do certificado digital disponível na URI especificada no parâmetro
\textit{f} da descrição da mídia, como ilustra-se no Trecho de
Código~\ref{algo:sdp-mediadesc}, Linha 8, da
...
conexão para obter um fluxo de dados \setpk.
