...
pacote alcançar o destino, o nó \serv utiliza $R_{p}$ para transmitir o fluxo de
dados, pois trata-se da menor taxa de transmissão do roteador mais
congestionado. Este procedimento se repete periodicamente utilizando os pacotes
de dados \pac{GMTP-Data} e \pac{GMTP-Ack}.}
...
instante $t$. Cada nó \repasss atualizada $R(t)$ aproximadamente a cada RTT.
  ...
  \item Todo pacote GMTP carrega duas informações de controle (campo no
cabeçalho):
...
% com quais nós devem fazer parcerias ou quais são seus melhores parceiros.
  ...
  \item No início de uma transmissão de um fluxo de dados \setpk, o nó \clis
transmite um pacote \pks com o valor de $R_{p} = \infty$ em direção a \serv.
...
\subsubsection{Segmentação de um caminho \setwayi e definição de $d_0$:}

Falar do problema de usar o RCP (end-to-end pode ser um problema) e o GMTP
segmenta isso

que, no caso do GMTP, não é definido entre o nó \servs
e o nó \cli, como na versão original do RCP. Em vez disso, o tempo $d_0$ é
definido entre dois nós \repasss e \repassu{q} parceiros entre si contidos em um
