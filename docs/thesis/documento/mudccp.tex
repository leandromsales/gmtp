...
os pacotes de dados \pks a uma taxa de \ut{4}{Mbps}. Como resultado dessa estratégia e ainda considerando o exemplo em discussão, o nó \transu{3} deverá solicitar ao nó \servu{1}, o fluxo de dados \setpks com codificação compatível com a taxa de bits de \ut{1}{Mbps} ou inferior e então servir ao nó \transu{1}. A partir deste ponto, o nó \transu{3} recebe a mídia codificada em duas taxas de bits: \ut{1024}{Kbps} e \ut{4096}{Kbps}. Dessa forma, \transu{3} torna-se capaz de servir a outros nós \repasss tanto a uma taxa de \ut{1}{Mbps} quanto a uma taxa de \ut{4}{Mbps}.

Como discutiu-se na Seção~\ref{subsubsec:desc-conteudo}, o GMTP permite descrever a mídia em múltiplas codificações, ao mesmo tempo que os nós \repasss podem acessar tal informação para implementar a solução discutida anteriormente. Como resultado dessa estratégia, pode-se afirmar que o GMTP realiza distribuição de mídias ao vivo com suporte à seleção de fluxos codificados em diferentes taxas de bits pelo nó \serv, selecionado de forma segmentada de acordo com a capacidade de transmissão de um sub-caminho entre dois nós repassadores. A decisão e as funções de adaptação da mídia é uma decisão de aplicação e está fora do escopo deste trabalho, porém, pode-se considerar o uso de estratégias avançadas para este propósito, como a apresentada em~\cite{Fernandes20111683}.
