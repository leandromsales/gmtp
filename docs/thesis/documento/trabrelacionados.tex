...
executaram simulações e pôde-se observar que a partir de 8 sub-fluxos, as duas
métricas citadas anteriormente não sofrem alterações positivas, em alguns casos
até pioram (quando se utiliza nós com capacidade heterogêneas de transmissão).
No GMTP, utiliza-se sempre o método push após um nó estabelecer uma conexão,
mas os roteadores no caminho entre o nó servidor e o nó cliente podem
interceptar os pedidos de conexão transmitidos por outros nós clientes.
Essa estratégia reduzir a quantidade de requisições ao servidor e o tempo para
