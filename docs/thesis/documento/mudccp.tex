...
As trocas de dados entre nós GMTP ocorrem por meio do envio e recebimento de partes de uma mídia (\textit{chunks}), transmitidas por diferentes nós da rede. O GMTP transmite os fluxos de dados em modo \textit{unicast} e compartilha-os (\textit{multi-unicast}) entre os roteadores quando estes sinalizam o interesse pela mídia. No último salto (rede local), o GMTP distribui os pacotes de dados aos nós clientes em modo \textit{multicast}. Em ambos os modos de transmissão, o GMTP realiza controle de congestionamento em transmissões sem garantia de entrega, ao passo que a escolha do modo de transmissão para disseminar um fluxo de dados ocorre sem a influência da aplicação, que precisa simplesmente ``sintonizar'' sua conexão em um determinado canal \textit{multicast} definido pelo roteador. Tal abstração para a camada de aplicação ocorre de modo que os processos em execução utilizam o GMTP através de uma API compatível com as especificações de socket BSD e POSIX, permitindo-se adaptações mais diretas dos atuais sistemas.
...
cliente conectado à rede, ou seja, ativo e operante.
...
dados, impedindo que a rede entre em congestionamento.
...
Assim, pode-se pré-selecionar nós parceiros filtrados por métricas que
...
transmissão unicast restrito apenas para transportar os fluxos de dados entre redes distintas, evitando-se a relação de uma conexão por cliente ao nó servidor. Este assunto será retomado na Seção~\ref{sec:asptransrecep}.

  \item Uso de algoritmo de controle de congestionamento assistidos pela
rede, em transmissões em modo unicast (GMTP-UCC); e uso do \textit{TCP Friendly Rate
Control} (TFRC) adaptado às transmissões de fluxos de dados em modo multicast (GMTP-MCC).
...
com notificação e reeleição de novos nós. Este e outros assuntos relacionados serão retomados na Seção~\ref{sec:outros-aspectos}.
...
% Note que para evitar sobrecarga de processamento nos roteadores, pode-se evitar
% falar aqui que nem todo pacote de reply precisa ser preenchido com o identificador, basta o roteador fazer isso uma vez em um certo período de tempo, voltando a fazer novamente quando este período expirar. Para detectar se deve ou não colocar o identificador no pacote de reply, basta fazer cache do endereço de origem/destino (a princípio não precisa cachear o nome da mídia).
...
em um nó \serv. Como ilustra-se na Figura~\ref{fig:conn-phase3-1}, para conseguir mais nós parceiros \repassu{q}, o nó \repassu{3} envia uma requisição do tipo \pac{GMTP-RelayQuery} para \servs e obtém um subconjunto de nós \repassu{q} candidatos a parceiro de \repassu{3}. O nó \servs constrói a lista de nós parceiros e envia ao nó \repass, funcionando como um indexador (\textit{tracker}) de nós parceiros \repassu{q}, pré-selecionando parcerios para \repass. No caso do exemplo supracitado, essa pré-seleção ajuda o nó \repassu{3} a escolher os melhores parcerios disponíveis, de acordo com os seguintes critérios de prioridade:
...
\begin{enumerate}
  ...
  \item um nó \repasss envia periodicamente requisições do tipo \pac{GMTP-RelayQuery} para o nó \servs a fim de descobrir melhores parceiros e aumentar o número de parcerias. Por se tratar de fluxos de dados ao vivo, não necessariamente quanto mais parcerios um nó \repasss tem, melhor será a qualidade do fluxo de dados \setpk. Por isso, um nó \repasss sempre mantém uma lista de candidatos a parcerios \repassu{q} fornecida pelo nó \serv, porém não necessariamente estabelece parcerias com todos. Em vez disso, executa-se repetidamente as seguintes ações:
...
transmitidas como resposta no pacote do tipo \pac{GMTP-Register-Reply}. Cada entrada disponível no pacote do tipo \pac{GMTP-RelayQuery-Reply} contém endereço IP do nó repassador candidado a parceiro e sua respectiva chave de autorização;
...
valores destes parâmetros são considerados padrões em aplicações tradicionais de
...
quantidade de parcerias e se registra tais informações na tabela de recepção de fluxos de dados, tal como ilustra-se Figura~\ref{fig:tabela-recepcao-fluxo}. Nesse exemplo, observa-se que um nó \repasss está recebendo e repassando aos seus nós \clis $\in$
...
\textit{83.67.132.41:53434}. Para esse fluxo de dados \setpk, o nó \repasss repassa os pacotes de dados \pks para os nós \clis $\in$ \subsetcli$($\repass$)$ através do canal multicast \textit{239.192.68.79:1900}.
...
Partindo desse ponto, Nandita et. al~\cite{Dukkipati:2008:RCP:1368746} argumentou em sua tese de doutorado
...
% por exemplo, o número de fluxos enviando pacotes de dados \pk;
    ...
    \item $\gamma$, tal que $0 < \gamma \leq 1$, controla o pico de utilização do canal. Por exemplo, se desejar utilizar no máximo \ut{95}{\%} do canal em um certo instante $\hat{t}$, basta definir $\gamma = 0.95$. Nesse caso, pode-se tratar cenários de reserva de banda para um ou mais fluxos de dados específicos;
