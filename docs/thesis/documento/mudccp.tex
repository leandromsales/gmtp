...
Nesse contexto, utiliza-se o Algoritmo~\ref{algo:registerRelay} para que
um nó \repasss se registrar a um nó \serv. Note que o nó \repasss não é obrigado
a informar qual fluxo de dados \setpks está interessado em obter, tal como
explicado anteriormente, lembre-se, o nó \repasss se comporta, por analogia,
como uma antena de TV. Porém, da mesma forma que é possível uma antena de TV
obter apenas o sinal correspondente a um determinal canal (frequência de TV),
também é possível que um nó \repasss especifique exatamente qual fluxo de dados
\setpks deseja começar a receber. Neste caso, o nó \servs executará um
procedimento para determinar se aceita ou não o pedido de conexão para
transmitir \setpk. Caso aceite, a transmissão do fluxo de dados \setpks entre
\servs e \repasss ocorrerá em modo unicast, diretamente entre eles, caso
contrário, o nó \servs delegará a transmissão de tal fluxo de dados \setpks para
algum outro nó repassador \repassu{q} que se tornará parceiro do
...
a given node \serv. If \pks is given, node \clis wants to receive the flow \setpk, so
notify \serv.}
...
\If(){\pks $\neq$ NULL} {
  ...
  \uIf(){\textit{channel} $\neq$ NULL} {
    ...
    \respondToClients{\textit{channel}}\tcc*[r]{See Section~\ref{subsec:conexao-requisicao}}
    \label{algo-line:respondToClients1}
    \Return{0}\;
  } \Else() {
      \tcc{Add \clis in the list of receivers waiting to receive \setpk, send request to \servs and wait registration reply. When received, executes onReceiveGMTPRegisterReply (Algorithm~\ref{algo:onReceiveGMTPRegisterReply}).}
%  If the \pac{GMTP-Register} was already sent, nothing should be done other than wait for \pac{GMTP-Register-Reply}.
	...
	\Return{0}\;
      ...
      \tcc{Ask \clis to wait until receive registration reply}
      \respondToClients{\GMTPRequestReply{\cli, \setpk}}\;
      \Return{0}\;
  }
}
...
aciona-se o Algoritmo~\ref{algo:onReceiveGMTPRegisterReply}, executando-se o
passos restantes explicados anteriormente.
...
\uIf(\tcc*[f]{\servs confirmed registration}){\pks = OK} {
  ...
  \If(\tcc*[f]{Reply to \clis $\in$ \subsetcli$($\repass$)$, waiting for \setpk}){\setpks $\neq$ NULL}{
    ...
    \tcc{Let \clis $\in$ \subsetcli$($\repass$)$ know the multicast channel to receive \setpk. See Section~\ref{subsec:conexao-requisicao}}
    \respondToClients{\GMTPRequestReply{\textit{channel}}}
    \label{algo-line:respondToClients2}
  }
  \tcc{It was just a reply of a registration.}
...
de parcerias apresentados a seguir, executados concorrentemente.
...
este caso, o nó \servs transmite apenas um único fluxo de dados \setpks para
...
Nesse contexto, o nó \servs executa o
Algoritmo~\ref{algo:findPartnerIntersectPath} para realizar a formação de
parcerias por intersecção. Tal algoritmo é executado quando o nó \servs recebe
um pedido de conexão originado por um nó \repasss para obter um fluxo de dados
\setpk. O nó \servs compara os caminhos conhecidos \setwayis $\in$ \setways com
o caminho \textit{\setwayiu{\repassconst_\repassi}} contido no pedido de conexão
...
\ways a interceptar os próximos pedidos de conexão para obter \setpk, caso
...
dados \setpks para permitir futuras parcerias intra ou por intersecção. Note que
após este procedimento, o nó \servs passa a conhecer o caminho
...
de dados \setpk, o endereço e porta em que tal fluxo deve ser entregue. Como
a transmissão de um fluxo de dados \setpks entre um nó \repasss e seus nós
\clis $\in$ \subsetcli$($\repass$)$ ocorrerá em modo multicast, faz-se
necessária apenas uma entrada no mapeamento do nó \repasss por fluxo de dados
...
centrada no sistema final \textit{(host centric networks)}. Sendo assim, essa
proposta quebra um paradigma que tradicionalmente os pacotes de dados de uma
aplicação são roteados com base no sistema final de destino. No caso do GMTP,
...
\repasss solicitante como se fosse o nó \servu{2}. Salienta-se
...
primeira acontece quando, por exemplo, um nó qualquer \cliu{1} $\in$
\subsetcli$($\repass$)$ deseja obter \setpks
transmitido por um nó \servu{1} e não existe nenhum outro nó
\clis $\in$ \subsetcli$($\repass$)$ em sua rede local recebendo \setpk. Já a
segunda fase acontece quando um outro nó \cliu{2} $\in$
\subsetcli$($\repass$)$ precisa obter o mesmo fluxo de dados \setpk, solicitado
previamente pelo nó \cliu{1}. E, por fim, a terceira fase acontece quando o nó
\repasss começa a buscar novos nós parceiros \repassu{q} a fim de obter
\setpks através de nós repassadores alternativos.

Na Figura~\ref{fig:processo-conexao}, ilustram-se um nó \serv, que gera um fluxo
de dados \setpk e 12 nós \repass, que constituem uma rede de diferentes domínios
administrativos, sendo o nó \repassu{1} o repassador de um desses domínios,
composto por 6 nós \clis $\in$ \subsetcli$($\repass$)$ (Rede Local).
...
de dados todas as vezes que receber um pacote do tipo \pac{GMTP-Request},
transmitido por um nó \clis $\in$ \subsetcli$($\repass$)$. Com base no estado da
referida tabela, o nó \repasss realiza uma determinada ação de repasse,
mediante a fase de conexão que o referido nó repassador se encontra com relação
ao fluxo de dados \setpks solicitado.
...
do processo de conexão do GMTP na Fase 1, considere:
...
Para obter o fluxo de dados \setpk, o nó \cliu{3} cria um socket GMTP e
transmite um pacote do tipo \pac{GMTP-Request}
(Figura~\ref{fig:processo-conexao-1}, Passo 1). Para construir o pacote do tipo
\pac{GMTP-Request}, qualquer nó \clis deve especificar o valor para o endereço
IP de destino como sendo o endereço do nó \servs que transmite \setpk, com o
valor para o campo do cabeçalho de rede \textit{TTL=1}. Alternativamente, o nó
\clis pode especificar o endereço IP de destino como sendo \textit{0.0.0.0}.
Além dos valores para o IP de destino e para o \textit{TTL}, o nó \clis também
deve informar o nome do fluxo de dados \setpks que o usuário deseja reproduzir,
sendo tal informação incapsulada no campo \textit{nome do fluxo de dados}
(\textit{data flow name}), presente no cabeçalho de transporte do pacote do tipo
\pac{GMTP-Request}. Ao definir o endereço IP de destino como sendo o IP de
\serv, permite-se que o roteador padrão de \cliu{1}, no caso o nó \repassu{1},
obter o endereço IP do nó \servs sem precisar consultar o servidor DNS. Caso
contrário, quando o IP especificado for \textit{0.0.0.0}, o nó \repassu{1}
deverá realizar tal consulta, como discutiu-se na
...
Na Fase $1$ do processo de estabelecimento de conexão, considera-se que não
existe nenhum nó \clis $\in$ \subsetcli$($\repassu{1}$)$ recebendo o fluxo de
dados \setpk.
Sendo assim, quando o pacote \pac{GMTP-Request} alcançar o nó \repassu{1}
(Passo 2 da Figura~\ref{fig:processo-conexao-1}), o nó \repassu{1} consulta a
tabela de recepção de fluxos de dados e constata que não há qualquer registro
para o fluxo de dados \setpk, pois a referida tabela estará vazia, como
apresentou-se na Figura~\ref{fig:tabela-recepcao-1},
Seção~\ref{subsec:tabela-recepcao}. Nesse instante, o nó \repasss inicia um
...
tipo \pac{GMTP-Register} em direção ao nó \servu{1} e, à medida que os nós
...
(Passo 3 da Figura~\ref{fig:processo-conexao-1}), conforme discutiu-se na
Seção~\ref{sec:descparc}. Na Figura~\ref{fig:processo-conexao-1}, ilustra-se o
caminho \setwayiu{1}, destacado na cor vermelha.
...
sintonizar o socket no canal multicast informado pelo nó \repassu{1}, este
começa a receber os pacotes de dados \pks do tipo \pac{GMTP-Data} ou
\pac{GTMP-DataAck} (Passo 5 da
Figura~\ref{fig:processo-conexao-1}). Este assunto será retomado na
Seção~\ref{sec:asptransrecep}.

O Algoritmo~\ref{algo:respondToClients},

\vspace{0.8cm}

\begin{algorithm}[H]
\label{algo:respondToClients}
\caption{respondToClients(channel: MulticastChannel)}
\SetAlgoLined
\tcc{A \repasss node executes this algorithm to handle the response to clients
after it receives a packet confirming the registration of participation and
updading the table of flow reception. Specifically, this function is invoked in
the Line~\ref{algo-line:respondToClients1} of
Algorithm~\ref{algo:registerRelay} and in
Line~\ref{algo-line:respondToClients2} of the
Algorithm~\ref{algo:onReceiveGMTPRegisterReply}.}

% \SetKwFunction{Union}{Union}\SetKwFunction{makePkt}{makePkt}
% \SetKwFunction{Union}{Union}\SetKwFunction{getPacketFieldValue}{
% getPacketFieldValue}
% \SetKwFunction{Union}{Union}\SetKwFunction{recvPktRdt}{recvPktRdt}
% \SetKwFunction{Union}{Union}\SetKwFunction{sendPktRdt}{sendPktRdt}
% \SetKwFunction{Union}{Union}\SetKwFunction{length}{length}
% \SetKwFunction{Union}{Union}\SetKwFunction{getKnownPathsOfFlow}{
% getKnownPathsOfFlow}
% \SetKwFunction{Union}{Union}\SetKwFunction{GMTPResponse}{GMTPResponse}
%
% \textit{\setpk} \attrib \getPacketFieldValue{\pk, `flow'}\tcc*[r]{Extracts
% \setpks in \pk}
% \textit{\setwayiu{\repassconst_\repassi}} \attrib \getPacketFieldValue{\pk,
% `path'}\tcc*[r]{Extracts \setwayiu{\repassconst_\repassi} in \pk}
%
% \textit{\setwayiu{\setpkc}} \attrib \getKnownPathsOfFlow{\setpk,
% \setway}\tcc*[r]{\setwayiu{\setpkc} $\subset$ \setway}
% % \tcc*[r]{For a given flow and all the known paths \setways in this \serv, get
% a sub set of paths used to transmit \setpk}
%
% \ForEach{\setwayis $\in$ \setwayiu{\setpkc}}{
%     \ForEach{\ways $\in$ \setwayi} {
%       \If(){\ways $\in$ \textit{\setwayiu{\repassconst_\repassi}}} {
% 	\tcc{The node \ways is common in \setwayis and in
% \textit{\setwayiu{\repassconst_\repassi}}.}
% 	\textit{done} \attrib \textit{true}\;
% 	break\;
%       }
%     }
%     \If(){\textit{done}} {
% %       \tcc{Create a \pac{GMTP-Response} and send it to \way. After receiving
% \textit{\pk}, \ways becomes a relay of the flow \setpk.}
% %       \pks \attrib \makePkt{\pac{GMTP-Response}(1), \ways}\;
%       \tcc{\servs stores \textit{\setwayiu{\repassconst_\repassi}} as a known
% path and replies to \repass, asking \ways to act as a relay for \setpk. \servs
% actives the flag 'relay' of the \pac{GMTP-Response} packet.}
%       \setway[\length{\setway}] \attrib
% \textit{\setwayiu{\repassconst_\repassi}}\;
%       \Return{\GMTPResponse{\way, relay=1}}\;
% %       \sendPktRdt(\pk)\;
% %       \exit{}\;
%     }
% }
% \tcc{\servs must register \textit{\setwayiu{\repassconst_\repassi}} as a known
% path and reply to \repasss by accepting its connection request, since no node
% \ways is intersecting \textit{\setwayiu{\repassconst_\repassi}}.}
% \setway[\length{\setway}] \attrib \textit{\setwayiu{\repassconst_\repassi}}\;
% \Return{\GMTPResponse{\repass, relay=0}}\;
% % \pks \attrib \makePkt{\pac{GMTP-Response}(0), \repass}\;
% % \sendPktRdt(\pk)\;
...
\respondToClients{\textit{channel}} % nessa tem que buscar os clientes esperando
por resposta
\respondToClients{\setpk, \cli}

tem que ter: getClientsWaitingForFlow(\setpk) mandar o leitor ver a linha 7 do
algoritmo algo:registerRelay
...
A fase $2$ de conexão ocorre quando futuras requisições a \setpks ocontecerem
...
\cli, por exemplo, \cliu{2}, usando o pacote do tipo \pac{GMTP-RequestNotify},
...
IP o nó \repasss passará a transmitir os dados (canal de repasse), e mais um
