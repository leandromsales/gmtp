...
%   \item Constataram-se melhorias de desempenho na transmissão de fluxos multimídia quando utilizado uma versão adaptada do RCP com o conceito de sub-fluxos, comparando-se com a versão normal do RCP.
  ...
  \item Apesar do GMTP não ser um protocolo com suporte a garantia de entrega, demonstrou-se e conclui-se que é possível transmitir pacotes de dados e oferecer uma boa qualidade de serviço às aplicações, ao combinar os métodos \textit{push/pull}, permitindo-se a obtenção de pacotes de dados, provavelmente perdidos, através do método \textit{pull}. Com isto, utiliza-se melhor os recursos de rede ao evitar super utilização dos canais com transmissão de fluxos de dados duplicados, mesmo em redes com altos níveis de congestionamento e uma certa taxa de perda de dados, como foi o caso do cenário adotado no experimento realizado e apresentado no Capítulo~\ref{cap:analisedesemp}.
...
%   ~\cite{1019258}
  ...
  \item O GMTP foi projetado para se permitir uma implantação progressiva, mas enquanto existirem roteadores que não o suportem, pode-se ocorrer medições equivocadas da largura de banda disponível. Por este motivo, prefere-se selecionar parceiros cujo caminho é completo.
...
% O Denacast é um sistema de distribuição de mídias ao vivo baseado no CoolStreaming com suporte a uma rede de distribuição de conteúdos (P2P/CDN). Com os resultados comparativos obtidos, contribui-se para tomada de decisões sobre futuras pesquisas no contexto de distribuição de mídias ao vivo. O CCN-TV é um sistema de TV para Internet com suporte às redes centradas na informação. Com isto, contribui-se para tomada de decisões sobre futuras pesquisas no contexto de distribuição de mídias ao vivo.
  ...
  \item A descrição detalhada do GMTP e a implementação de artefatos de software que permitem a reprodução do ambiente de estudo em um simulador de rede OMNet++~\cite{XU2011,omnetpp2014}, bem como a metodologia adotada para executar o experimento.
