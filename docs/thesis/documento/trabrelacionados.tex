...
parte de um todo, com similaridades relevantes se comparado ao \mudccp. A compilação dos trabalhos a seguir foi realizada com base em informações obtidas e adaptadas de publicações encontradas em diversas fontes (revistas, conferências, livros, teses e dissertações) disponíveis na literatura.

Para uma leitura linear deste capítulo, na próxima seção, apresenta-se uma breve discussão sobre os principais marcos na área de distribuição de mídias ao vivo, posicionando o GMTP em uma ilustração macro sobre o estado da arte. Em seguida, na Seção~\ref{sec:proto-app-ea}, apresenta-se detalhes dos principais sistemas e protocolos de distribuição de mídias ao vivo, enfatizando suas respectivas arquiteturas e modelos de serviço. Já na Seção~\ref{sec:trab-redes-centradas-informacao}, apresenta-se uma nova vertente de pesquisa sobre distribuição de dados na Internet, conhecida por Redes Centradas na Informação, enfatizando suas principais características e os trabalhos relacionados à distribuição de mídias ao vivo. E, por fim, na Seção~\ref{sec:trabrel-sumario}, apresenta-se um breve resumo sobre este capítulo. Caso o leitor deseje realizar uma leitura mais focada e levando em consideração sua experiência sobre distribuição de mídias ao vivo, recomenda-se a leitura apenas das seções que abordam os trabalhos mais fortemente relacionados ao GMTP. Nesse caso, recomenda-se a leitura da Seção~\ref{subsec:coolstreaming}, onde se discute sobre o projeto CoolStreaming; a Seção~\ref{subsec:denacast}, onde se discute sobre o projeto Denacast e, por fim, toda a Seção~\ref{sec:trab-redes-centradas-informacao}.
...
Nesse contexto, ilustra-se na Figura~\ref{fig:gmtp-posicao}, os principais representantes e marcos importantes no contexto de distribuição de mídias ao vivo, bem como a evolução das arquiteturas e modelos de serviços adotados na Internet (incluindo o GMTP). Como pode-se observar, atualmente existem duas vertentes para transmissão de dados na Internet: as Redes IP e as Redes Centradas na Informação. Para as soluções disponíveis nas redes IP, destacam-se o sistema DONet/CoolStreaming~\cite{xinyan_zhang_coolstreaming/donet:_2005}, o Denacast~\cite{5764567} e o protocolo DASH~\cite{6077864}, detalhados na Seção~\ref{sec:proto-app-ea}.

Até 2004, predominaram soluções de transmissão de mídias ao vivo baseado no modelo de serviço cliente/servidor. Em seguida, com base nas pesquisas sobre compartilhamento de arquivos considerando o modelo de serviço P2P, estenderam-se tal modelo de serviço para distribuição de mídias ao vívo. Naquela época (2005), destacou-se o projeto CoolStreaming, sendo o primeiro capaz de distribuir um mesmo evento para aproximadamente 40 mil nós clientes. Em 2008, os autores propuseram melhorias arquiteturais, propondo-se a mudança do método \textit{pull} para um o método \textit{push-pull} de \textit{download} das partes de uma mídia. Em paralelo, entre 2002 e 2005, publicaram-se as primeiras soluções sobre um protocolo de controle de congestionamento assistido pela rede chamado XCP~\cite{Katabi2003}. As pesquisas também evoluíram, quando propuseram os protocolos RCP (2008)~\cite{Dukkipati:2008:RCP:1368746} e o VCP~\cite{4460574}, após identificarem algumas limitações do XCP. Desde então, as pesquisas nesse sentido desaceleraram, sem mais nenhuma proposta contundente nesse contexto. Em 2011, dentre as várias soluções para distribuição de mídias ao vivo, pode-se destacar a Denacast, propondo-se modelo híbrido P2P/CDN para distribuição de mídas ao vivo com base no CoolStreaming. Entre 2012 aos dias atuais, existem diversas pesquisas para distribuição de mídias ao vivo através do uso do protocolo HTTP/TCP, permitindo-se que os nós clientes obtenham as partes da mídia que mais se adequar a sua capacidade de recepção, ao passo que a infra-estrutura de rede é baseada em diferentes modelos de serviço. Nesse último caso, destacam-se o uso exclusive grandes redes de distribuição de conteúdo (CDN), bem como soluções híbridas, P2P/CDN com base em HTTP. Até o presente momento, o foco é completamente em soluções na camada de aplicação.

Com relação às Redes Centradas na Informação, em 2006 Van Jacobson introduziu o assunto em uma palestra proferida para um grupo seleto de pesquisadores de várias partes do mundo, no GooglePlex, Montain View, Califórnia~\cite{VanJ2006}. Argumentou-se que o modelo de requisição origem/destino empregado na Internet não mais se sustenta, considerando o aumento significativo do tráfego de dados atual, dando início à uma nova vertente de pesquisa que ficou conhecida por Redes Centradas na Informação ou ICN (\textit{Information Centric Networks}). A verdade é que alguns autores argumentam que essa vertente de pesquisa já existia desde os anos 1970, mas sem evolução até 2006. A partir de 2006, surgiram as seguintes propostas: DONA (\textit{Data Oriented Network Architecture}), PSI (\textit{Publish-Subscribe Internet Routing}), NetInf (\textit{Network of Information}) e o CCN (\textit{Content-Centric Networks}), atualmente conhecida pela sigla NDN (\textit{Named-Data Networks}). Em 2013, propuseram-se o CCN-TV, o primeiro sistema para distribuição de mídias ao vivo em NDN e com suporte ao DASH. Na Seção~\ref{sec:trab-redes-centradas-informacao}, discute-se mais detalhes sobre as ICN, em especial a NDN. Além disso, destaca-se as propostas para distribuição de mídias ao vivo, com destaque ao CCN-TV.

Não obstante, neste trabalho, propõe-se o GMTP, apresentado em versão incipiente no final de 2011 e neste trabalho em sua versão aprimorada. No restante deste capítulo, confronta-se diversas propostas para distribuição de mídias ao vivo com o GMTP, ao passo que no Capítulo~\ref{cap:mudccp}, detalha-se seu funcionamento. Dentre todas as soluções discutidas, considera-se o CoolStreaming/Denacast e o CCN-TV as propostas fundamentalmente mais próximas ao GMTP. Por esse motivo, no Capítulo~\ref{cap:analisedesemp}, compara-se em mais detalhes o desempenho de ambas em comparação ao GMTP.
...
ao passo que outras propostas ainda estão em estágio embrionário~\cite{6231279,6563278,6231276,5936152,6384450}. Como a NDN apresenta diferenças substanciais em seu modelo de serviço para prover o conteúdo de interesse e funções disponíveis no núcleo da rede, a seguir, detalhe-se seu funcionamento e suas atuais aplicações no contexto de distribuição de mídias ao vivo.
