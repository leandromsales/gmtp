...
descritos anterioremente que o nó \servs executa a fim de realizar a formação de
parcerias por intersecção. No GMTP, executa-se tal algoritmo quando o nó \servs
recebe um pedido de conexão originado por um nó \repasss para obter um fluxo de
dados \setpk. O nó \servs compara os caminhos conhecidos \setwayis $\in$
\setways com o caminho \textit{\setwayiu{\repassconst_\repassi}} contido no
pedido de conexão transmitido por \repass. Se \servs determinar um nó \ways em
comum entre algum \setwayis e \textit{\setwayiu{\repassconst_\repassi}}, \servs
instrui o nó \ways a interceptar os próximos pedidos de conexão para obter
\setpk. Caso contrário, \servs aceita o novo pedido de conexão e todo o caminho
\textit{\setwayiu{\repassconst_\repassi}} passa a ser alimentado com o fluxo de
dados \setpk, permitindo-se futuras parcerias intra ou por intersecção. Note que
...
and \serv. The packet \pks carries \setwayiu{\repassconst_\repassi} and the name
for \setpk.}
...
pacote do tipo \pac{GMTP-Request}, transmitido por um nó \clis $\in$
...
dados \setpk. Sendo assim, quando o pacote \pac{GMTP-Request} alcançar o nó \repassu{1}
...
Para isto, o nó \repassu{1} intercepta a requisição do nó \clisu{2}, consulta a
tabela de recepção de fluxos de dados e dessa vez constata a recepção do fluxo
de dados \setpk, criando-se o pacote do tipo \pac{GMTP-Request-Reply}. Este
