 ...
 *  and to make it work as a loadable module in the GMTP stack written by
...
#ifdef CONFIG_IP_GMTP_CCID3_DEBUG
static bool ccid3_debug;
#define ccid3_pr_debug(format, a...)	GMTP_PR_DEBUG(ccid3_debug, format, ##a)
#else
				  ...
				  enum ccid3_hc_tx_states state)
{
	...
	ccid3_pr_debug("%s(%p) %-8.8s -> %s\n",
		       gmtp_role(sk), sk, ccid3_tx_state_name(oldstate),
 ...
 *	@len: GMTP packet payload size in bytes
 *
 ...
 *	As elsewhere, RTT > 0 is assumed by using gmtp_sample_rtt().
 */
...
static int ccid3_hc_tx_send_packet(struct sock *sk, struct sk_buff *skb)
{
	...
	dp->gmtps_hc_tx_insert_options = 1;
	GMTP_SKB_CB(skb)->gmtpd_ccval  = hc->tx_last_win_count;
...
static void ccid3_hc_tx_packet_sent(struct sock *sk, unsigned int len)
{
	...
	if (tfrc_tx_hist_add(&hc->tx_hist, gmtp_sk(sk)->gmtps_gss))
...
static void ccid3_hc_tx_packet_recv(struct sock *sk, struct sk_buff *skb)
{
	...
	if (!(GMTP_SKB_CB(skb)->gmtpd_type == GMTP_PKT_ACK ||
	 ...
	 */
	acked = tfrc_tx_hist_find_entry(hc->tx_hist, gmtp_hdr_ack_seq(skb));
	if (acked == NULL)
		return;
	...
	now	  = ktime_get_real();
	r_sample  = gmtp_sample_rtt(sk, ktime_us_delta(now, acked->stamp));
...
done_computing_x:
	ccid3_pr_debug("%s(%p), RTT=%uus (sample=%uus), s=%u, "
			       ...
			       gmtp_role(sk), sk, hc->tx_rtt, r_sample,
	 ...
	 * that we now can send a packet, so wake up gmtp_wait_for_ccid
	 */
	...
	ccid3_pr_debug("%s(%p), Scheduled no feedback timer to "
		       ...
		       gmtp_role(sk), sk, usecs_to_jiffies(t_nfb), t_nfb);
				     ...
				     u8 option, u8 *optval, u8 optlen)
{
	...
	case TFRC_OPT_LOSS_EVENT_RATE:
		...
		if (packet_type == GMTP_PKT_DATA)
			break;
		if (unlikely(optlen != 4)) {
			GMTP_WARN("%s(%p), invalid len %d for %u\n",
				  gmtp_role(sk), sk, optlen, option);
			return -EINVAL;
		}
				  ...
				  u32 __user *optval, int __user *optlen)
{
	...
	switch (optname) {
	case GMTP_SOCKOPT_CCID_TX_INFO:
		if (len < sizeof(tfrc))
			return -EINVAL;
				  ...
				  enum ccid3_hc_rx_states state)
{
	...
	ccid3_pr_debug("%s(%p) %-8.8s -> %s\n",
		       gmtp_role(sk), sk, ccid3_rx_state_name(oldstate),
				      ...
				      enum ccid3_fback_type fbtype)
{
	...
	case CCID3_FBACK_PERIODIC:
		...
		if (delta <= 0)
			GMTP_BUG("delta (%ld) <= 0", (long)delta);
		else
	...
	hc->rx_last_counter	    = gmtp_hdr(skb)->gmtph_ccval;
	hc->rx_bytes_recv	    = 0;

	dp->gmtps_hc_rx_insert_options = 1;
	gmtp_send_ack(sk);
}
...
static int ccid3_hc_rx_insert_options(struct sock *sk, struct sk_buff *skb)
{
	...
	if (gmtp_insert_option(skb, TFRC_OPT_LOSS_EVENT_RATE,
			       &pinv, sizeof(pinv)) ||
	    gmtp_insert_option(skb, TFRC_OPT_RECEIVE_RATE,
...
static u32 ccid3_first_li(struct sock *sk)
{
	...
	if (hc->rx_rtt == 0) {
		GMTP_WARN("No RTT estimate available, using fallback RTT\n");
	...
	if (x_recv == 0) {		/* would also trigger divide-by-zero */
		GMTP_WARN("X_recv==0\n");
		if (hc->rx_x_recv == 0) {
			GMTP_BUG("stored value of X_recv is zero");
			return ~0U;
		}
		x_recv = hc->rx_x_recv;
	}
...
static void ccid3_hc_rx_packet_recv(struct sock *sk, struct sk_buff *skb)
{
	...
	const u64 ndp = gmtp_sk(sk)->gmtps_options_received.gmtpor_ndp;
	const bool is_data_packet = gmtp_data_packet(skb);

	if (unlikely(hc->rx_state == TFRC_RSTATE_NO_DATA)) {
		if (is_data_packet) {
			const u32 payload = skb->len - gmtp_hdr(skb)->gmtph_doff * 4;
	 ...
	 */
	if (SUB16(gmtp_hdr(skb)->gmtph_ccval, hc->rx_last_counter) > 3)
				  ...
				  u32 __user *optval, int __user *optlen)
{
	...
	switch (optname) {
	case GMTP_SOCKOPT_CCID_RX_INFO:
