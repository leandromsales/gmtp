...
em $R_{p}$. Porém, essa estratégia pode limitar alguns nós de receberem os
...
abstraindo-se os nós clientes. Nesse cenário, observa-se um caminho \setwayi $=$
$\{$\transu{1}, \wayu{2}, \transu{3}, \wayu{4}$\}$. Isto significa que existem
nós \clis $\in$ $($\subsetcli(\transu{1}) $\cup$ \subsetcli(\transu{3})$)$
recebendo os pacotes de dados \pk. Ao utilizar apenas o RCP, o nó \servs
transmitirá pacotes de dados \pks a uma taxa de transmissão de \ut{1}{Mb/s}
tanto para os nós \clis $\in$ \subsetcli(\transu{1}) quanto para os nós \clis
$\in$ \subsetcli(\transu{3}). Ora, isso faz sentido apenas para os nós \clis
$\in$ \subsetcli(\transu{1}) e não para os nós \clis $\in$
...
nós \clis $\in$ \subsetcli(\transu{3}), já que neste o valor de $R(t)$
corresponde a \ut{8}{Mb/s}.
...
sub-caminho \setwayiua{1}{'} $=$ $\{$\transu{1}, \wayu{2}, \transu{3}$\}$ a
taxa de transmissão de \transu{3} em direção a \transu{1} será de \ut{1}{Mb/s},
ao passo que no sub-caminho \setwayiua{2}{'} $=$ $\{$\transu{3}, \wayu{4},
\servu{1}$\}$ será de \ut{4}{Mb/s}. Sendo assim, os nós \clis $\in$
...
parceiros de um nó solicitante \repass. Um dos critérios para sugerir nós
parceiros \repassu{q} é priorizar nós \repassu{q} que fazem parte de um
caminho \setwayis com maiores capacidade de transmissão. No GMTP isto é possível
porque os nós \servs conhecem a capacidade de transmissão de todo o caminho,
inclusive os pontos de congestionamento, como explicado anteriormente. Sendo
assim, dependendo da posição de um nó solicitante \repasss em um caminho
\setwayi, o nó \servs determina os parceiros \repassu{q} e sugere ao nó \repasss
através de um algoritmo simples de intersecção de caminhos, utilizando como
custo a capacidade de transmissão dos caminhos conhecidos \setway.
...
computação por cada pacote no roteador. Tanto o RCP quanto o XCP são os
protocolos mais conhecidos do estado da arte que tentam emular um PS e, por
