...
{
	return gmtp_sk(sk)->gmtps_tx_qlen &&
	       sk->sk_write_queue.qlen >= gmtp_sk(sk)->gmtps_tx_qlen;
}
...
static bool qpolicy_prio_full(struct sock *sk)
{
	if (qpolicy_simple_full(sk))
		gmtp_qpolicy_drop(sk, qpolicy_prio_worst_skb(sk));
	return false;
}

/**
 * struct gmtp_qpolicy_operations  -  TX Packet Dequeueing Interface
...
{
	qpol_table[gmtp_sk(sk)->gmtps_qpolicy].push(sk, skb);
}

bool gmtp_qpolicy_full(struct sock *sk)
{
	return qpol_table[gmtp_sk(sk)->gmtps_qpolicy].full(sk);
}
...
{
	return qpol_table[gmtp_sk(sk)->gmtps_qpolicy].top(sk);
}
...
bool gmtp_qpolicy_param_ok(struct sock *sk, __be32 param)
{
	/* check if exactly one bit is set */
	if (!param || (param & (param - 1)))
		return false;
	return (qpol_table[gmtp_sk(sk)->gmtps_qpolicy].params & param) == param;
}
