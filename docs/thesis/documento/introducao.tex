...
Esse cenário tem se comprovado nos últimos anos, propondo-se demasiadamente e recorrentemente diversas soluções para os problemas causados pela falta de um protocolo que generalize os serviços comuns dos sistemas de distribuição de mídias ao vivo. Por exemplo, no caso dos \textit{middlewares}, implementam-se funções para tratar aspectos de conexão multi-ponto e diferentes topologias~\cite{4404148,5158452,Wu:2008:ELP:1386109.1386112,4215750,5061910,Garroppo:2012:TCS:2160187.2160451,4251077,4644134}, seleção de
...
e segurança~\cite{vieira_borges_2010,4539702,4738451,865073,1388342}. Sendo assim, questiona-se a necessidade de se tratar os problemas supostamente resolvidos por essas funções e, nesse ínterim, identificar quais desses serviços poderiam ser melhor aproveitados se posicionados corretamente na pilha de protocolos TCP/IP.

O fato é que os temas supracitados foram ganhando importância ao longo dos anos porque os serviços de transmissão de mídias ao vivo se popularizaram, ao passo que surgiram mais necessidades de sofisticar as aplicações a fim de melhorar seu desempenho em entregar os fluxos de dados para uma população crescente de nós clientes. Apesar da arquitetura da Internet ser modularizada em camadas funcionais, ao longo dos anos as soluções promovidas no contexto dos referidos temas não foram disponibilizadas estrategicamente nas camadas corretas, mas sim apenas na camada de aplicação. Isto se explica pelo fato de que é muito mais fácil implementar uma solução na camada de aplicação do que em outras camadas mais abaixo, não apenas por aspecto de facilidade de desenvolvimento, mas também de implantação em larga escala na Internet, pois todas as outras camadas estão implementadas do sistema operacional para baixo.

Um exemplo do problema em discussão é a proposta ALM (\textit{Application Layer Multicast})~\cite{Banerjee:2002:SAL:964725.633045}. A princípio, uma maneira eficiente de se estruturar e enviar um fluxo de vídeo a um grupo de usuários na Internet era utilizar \textit{multicast} em nível de IP~\cite{guo_survey_2008}. Nesse caso, um nó fonte transmissor de uma mídia ao vivo se torna a raiz de uma árvore \textit{multicast} e os clientes recebem o conteúdo do vídeo através dos vários nós desta árvore, formada pelos roteadores que suportam \textit{multicast} em nível de IP. Apesar de melhorar escalabilidade no número de receptores do mesmo conteúdo ao vivo, tal estratégia foi considerada suficiente devido à complexidade de habilitá-la e mantê-la, principalmente em grande escala~\cite{castro_splitstream:_2003}. Para contornar a dificuldade em habilitar o modo \textit{multicast} em nível de IP, propuseram-se a arquitetura ALM, que é equivalente ao IP \textit{multicast}, porém implementada em nível de aplicação. A diferença entre o \textit{multicast} IP e o ALM é que no primeiro os roteadores duplicam e repassam os datagramas para os próximos roteadores até alcançar os sistemas finais, ao passo que no ALM os sistemas finais executam as ações citadas anteriormente, mas ainda dependem de funções de rede de mais baixo nível, o que pode aumentar o tempo de processamento e a complexidade da aplicação. Existes diversos \textit{middlewares} baseados em ALM e estes implementam suas próprias interfaces de uso e funcionalidades, incompatíveis entre
si~\cite{Yeo:2003:FMV:947820.947823,Banerjee:2006:OEO:1141103.1648579,1258118,jannotti_overcast:_2000,Kwon:2005:POM:1052084.1648646,1575808,castro_splitstream:_2003}. Apesar de todos esses esforços, questão de interoperabilidade dos sistemas de distribuição de mídias ao vivo vem à tona mais uma vez.
...
Para instanciar a discussão anterior, considere o caso do protocolo DCCP, proposto como um protocolo de transporte para transmissão de mídias ao vivo na Internet. O fato é que o DCCP é um protocolo orientado à conexão e portanto para cada novo usuário interessado em receber um fluxo multimídia transmitido, uma nova conexão se faz necessária, não havendo a possibilidade de compartilhamento entre diferentes nós interessados pelo mesmo conteúdo, pelo menos de forma automática e transparente para as aplicações. Este cenário é motivador para os desenvolvedores de aplicações proporem recorrentemente suas próprias soluções para distribuição de mídias ao vivo, enquanto o foco deveria ser entregar o fluxo de dados ao nó do usuário final da forma mais eficiente possível e definitiva e nesse trabalho argumenta-se que isso somente será possível se existir uma solução que promova a independência entre os sistemas. É evidente que os sistemas atuais podem passar a utilizar o DCCP para transportar os segmentos de mídias ao vivo, mas o DCCP apresenta problemas críticos quanto a escalabilidade devido ao mecanismo de conexão e proposta de controle de congestionamento fim-a-fim.
...
Diante do exposto, pretende-se responder a seguinte questão de pesquisa: \textit{como distribuir conteúdos multimídia ao vivo em larga escala abstraindo-se a complexidade e a interoperabilidade entre os sistemas com base no princípio da independência dos serviços de rede?} Uma pergunta secundária, mas não menos importante é: \textit{quais e como deveriam funcionar os serviços disponíveis na infraestrutura de rede para dar suporte à distribuição de mídias ao vivo se a Internet fosse proposta nos dias atuais, mas considerando-se os conhecimentos adquiridos com a evolução desses sistemas ao longo dos últimos anos?}
...
Diante desta hipótese, remete-se esta pesquisa ao estudo de técnicas e mudança na perspectiva da execução de serviços de rede em camadas inferiores a da aplicação, buscando-se melhorias substanciais na execução dos sistemas de distribuição de mídia ao vivo, permitindo-se que a rede desempenhe um papel mais efetivo nesse processo.

\section{Objetivos}

Neste trabalho, o objetivo é o projeto e a avaliação de um protocolo de rede denominado \textit{Global Media Transmission Protocol} (GMTP), que considera uma arquitetura híbrida P2P/CDN para distribuição de mídias ao vivo através da Internet. Mais especificamente, propõe-se um protocolo que explora a relação entre o escalonamento de recursos computacionais em redes P2P, a estabilidade das redes CDN e estratégias (algoritmos) que possam mitigar a complexidade no processo de construção dos sistemas de distribuição de mídias ao vivo na Internet.

O objetivo inicial é dar um passo atrás e repensar \textit{distribuição de mídias ao vivo na Internet}, entendendo-se que a interoperabilidade dos sistemas é uma premissa e interesse geral dos sistemas para reproduzir o conteúdo ao usuário final. Isto significa permitir que os pacotes de dados referentes a uma mídia ao vivo alcancem os sistemas finais o mais rápido possível, sem se preocupar sobre restrições comerciais para acessá-la.
