...
Desde da criação da Internet, diversos módulos complementares foram disponibilizados para atender as necessidades dos usuários, como o NAT, DNS com  suporte a balanceamento de carga, etc. Entretanto, essas soluções criaram seus próprios problemas~\cite{moore2002}, que foram resolvidos por outras proposta~\cite{RFC3489} ou utilizadas de forma 
errada~\cite{Vixie:2009:DNS:1610252.1610269}. Em geral, essas mudanças tem levado a situações complexas de gerenciamento de rede, ao passo que tem  sido disponibilizados avanços incipientes e lentos com foco no núcleo da rede. No entanto, uma nova vertente de pesquisa tem surgido com foco em propor a  próxima arquitetura de serviço para a Internet. Nesse contexto, considera-se o princípio de que os nós da rede devem trocar dados com base em qual informação se tem interesse em obter, e não sobre sua localização, definindo-se uma nova sub-área de estudo conhecida por Redes Centradas na Informação, ou originalmente, \textit{Information Centric Networks (ICN)} (Figura~\ref{fig:icn-abstracao}).

\begin{figure}
  \centering
  \subfigure[Telefonia: nomeia o caminho.] {
    \label{fig:icn-telefonia}
    \includegraphics[natwidth=664,natheight=312,scale=0.4]{imgs/trabrel/icn-telefonia.png}
  }
  \qquad
  \subfigure[IP: nomeia os sistemas finais.] {
    \label{fig:icn-ip}
    \includegraphics[natwidth=998,natheight=374,scale=0.275]{imgs/trabrel/icn-ip.png}
  }
  \qquad
  \subfigure[ICN: nomeia o conteúdo.] {
    \label{fig:icn-ndn}
    \includegraphics[natwidth=1008,natheight=380,scale=0.275]{imgs/trabrel/icn-ndn.png}
  }
...
funções de transporte e decisões de repasse com base no nome do conteúdo e não 
mais em sua localização. Isto significa que um roteador de rede pode fazer 
\textit{cache} de um conteúdo bastante acessado e retorná-lo quando houver 
interesse por mais de um nó da rede. Atualmente (2014), todas as propostas para uma nova arquitetura para a Internet estão em estágio inicial de desenvolvimento, de modo que seus modelos de serviço e interfaces de transporte estão longes de uma versão final. Por isso, os benefícios de utilizar tais arquiteturas para a distribuição de vídeo ao vivo são questionáveis e ainda não foram apresentados resultados contundentes sobre seu uso em larga escala na Internet. Por exemplo, é impossível para um nó receptor adaptar o 
fluxo de dados de acordo com as características de uma rota de rede, pois as 
partes de um vídeo podem ser servidas por diferentes nós de \textit{cache}. De 
fato, suportar a distribuição eficiente de mídias ao vivo usando ICN ainda 
necessita de várias melhorias no projeto arquitetural, por exemplo, desenvolver 
protocolos de transporte estendendo as interfaces atuais e considerando o 
paradigma atual de transmissão fim-a-fim~\cite{6691438}.

Dentre as propostas de uma nova arquitetura para a Internet com base no princípio das ICNs, destacam-se a \textit{Named-Data Networks (NDN)\footnote{\textit{Named-Data 
Networks}: nova denominação para o que originalmente ficou conhecido por 
\textit{Content-Centric Networks}.}}~\cite{6419703,Jacobson:2012:NNC:2063176.2063204, 6419703,6386696} ou Redes de Dados Nomeados e a \textit{Publish-Subscribe Internetworking 
(PSI)}~\cite{6231279} ou Redes de Publicação e Subscrição. Existem outras 
propostas em estágio embrionário~\cite{6563278,6231276,5936152,6384450}, sendo a 
CCN e a PSI as mais evoluídas, com diferenças substanciais em seus 
modelos de serviço para prover o conteúdo de interesse e suas funções 
disponíveis no núcleo da rede. Por esse motivo, nas próximas seções, discute-se 
em mais detalhes sobre tais propostas e suas aplicações no contexto de 
distribuição de mídias ao vivo.

\subsection{Redes de Dados Nomeados (NDN)}

A arquitetura NDN, originalmente conhecida por \textit{Content-Centric Network 
(CCN)}, nomeia os pacotes de conteúdos e prover um modelo de serviço orientado 
a pedido e resposta, onde os nós obtêm os pacotes de dados a partir da 
rede, não necessariamente a partir dos nós finais. Como ilustra-se na 
Figura~\ref{fig:ccn-fib-pit}, os nós requisitam os pacotes de dados enviando 
uma requisição informando o nome do conteúdo de interesse, que o identifica 
unicamente, em um pacote especial chamado de \textit{Interest Packets} (Pacotes 
de Interesse). Como resposta, os nós recebem os pacotes de dados com o
conteúdo (\textit{payload}) requisitado. Toda interação ocorre sem o uso de 
endereçamento de origem/destino. Para cada envio de pacote de interesse, o 
nó recebe um pacote de dados, então existe uma estrita relação um-para-um 
entre os pacotes de interesse e os pacotes de dados. A definição dos nomes dos 
conteúdos seguem uma estrutura similar às URIs: os nomes são definidos em 
hierarquias com componentes de tamanhos variados, por exemplo, 
\textit{/a/b/c.mp4}.
...
Isto acontece com o uso de três estruturas da dados, definidas por:
...
6666771
Peer-to-peer live adaptive video streaming for Information Centric
cellular networks
...
Jacobson:2009:VVC:1658978.1658980
VoCCN: Voice-over Content-centric Networks

IEEE COMSOC MMTC E-Letter, July 2013
...
Supporting Diverse Traffic Types in Information Centric 
...
A Push-Enabling Scheme for Live Streaming System 
