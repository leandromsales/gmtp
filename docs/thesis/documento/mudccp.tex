\chapter{Global Media Transmission Protocol}
\label{cap:mudccp}
...
suporte ao GMTP]{Tela da ferramenta de administração da distribuição Linux OpenWRT com
...
\end{center}
\vspace{-0.5cm}
\caption[Fluxograma geral do GMTP]{Fluxograma geral do GMTP, desde a solicitação do cliente por uma mídia ao vivo até receber uma resposta, executando-se ações importantes como registro de participação, controle de congestionamento, eleição de relatores e verificação de autenticidade dos dados.}
...
Para melhor organizar as discussões sobre o funcionamento do GMTP, nesta seção, descrevem-se suas definições, relações e restrições. Para isto, faz-se uso de fundamentos de teoria de conjuntos e teoria dos grafos~\cite{Jonathan2003,Seroul2000,Courant1996,Devlin1979}.
...
\caption[Registro de participação de um nó repassador ao servidor]{registerRelay(\serv: PeerServer, \pks $=$ \pac{GMTP-Request})}
\SetAlgoLined
...
\caption[Tratamento da mensagem do tipo \textit{Register Reply} para um registro de participação]{onReceiveGMTPRegisterReply(\pks $=$ \pac{GMTP-Register-Reply})}
\SetAlgoLined
...
\end{center}
\vspace{-1cm}
\caption[Exemplo de uma tabela de recepção de fluxo mantida por um repassador]{Exemplo de uma tabela de recepção de fluxo mantida por um repassador.}
...
\SetAlgoLined
\caption[Avalia intersecção entre caminhos para formar parcerias]{handleRegisterParticipation(\repass: PeerRelay, \pks $=$
\pac{GMTP-Register})}
...
\caption[Trata a resposta e responde aos cliente aguardando pelo conteúdo]{respondToClients(\pk: \pac{GMTP-RequestNotify})}
\SetAlgoLined
...
\end{center}
\vspace{-1cm}
\caption[Limitação do RCP no cenário adotado no GMTP.]{O RCP utiliza uma abordagem fim-a-fim para determinar a taxa de transmissão de um nó, porém isto pode limitar alguns clientes a receberem os pacotes de dados em uma taxa maior. Nesse caso, o nó \transu{3} tinha capacidade para receber o conteúdo a uma taxa de transmissão de \ut{4}{Mbps}, porém a taxa de máxima relatada por \transu{1} é de \ut{1}{Mbps}, fazendo com que todos os nós no caminho \setwayis recebam o fluxo de dados \setpks a \ut{1}{Mbps}.}
...
\end{center}
\vspace{-1cm}
\caption[Funcionamento básico do funcionamento do GMTP-UCC (\textit{Unicast Congestion Control})]{O GMTP-UCC segmenta o caminho e dessa forma não limita a taxa de transmissão de um nó capaz de receber um fluxo de dados em uma taxa de transmissão maior.}
...
\end{center}
\vspace{-1cm}
\caption[Evolução dos números de sequência de fluxos de dados, quando utilizando TCP (Reno), XCP e RCP]{Evolução dos números de sequência de fluxos de dados, quando utilizando TCP (Reno), XCP e RCP. O tamanho do fluxo no primeiro gráfico foi de 230 pacotes, e no segundo gráfico foi de 3600 pacotes. Extraído de~\cite{Dukkipati:2008:RCP:1368746}.}
...
\end{center}
\vspace{-1cm}
\caption[Evolução do RCP, XCP e TCP pelo tempo de finalização de um fluxo]{Tempo médio, em segundos, de finalização de um fluxo de dados, ao
...
Apesar de ser uma estratégia interessante e funcionar em conexões \textit{unicast}, em transmissões \textit{multicast} o algoritmo descrito anteriormente não é eficiente. O algoritmo é limitado devido a um problema conhecido por \textit{implosão de confirmações} (\textit{feedback implosion}). Esse problema ocorre quando há muitos receptores enviando relatórios de perdas para o mesmo transmissor, o que resulta em uma inundação de relatórios, os quais o transmissor é incapaz de processar em tempo hábil.
...
\serv, respectivamente. No Algoritmo~\ref{algo:digitalSignPacket},
apresenta-se o procedimento de assinatura de um pacote \pks $\in$ \setpks
adotado no GMTP, utilizando-se a técnica de criptografia \textit{hash} e assinatura digital~\cite{efficiente-hash-chain,lamport,802657}.

\vspace{0.5cm}

\begin{algorithm}[H]
\label{algo:digitalSignPacket}
\caption[Assina digitalmente o conteúdo do pacote]{digitalSignPacket(\pk: \pac{GMTP-Data})}
...
Um nó \repasss obtém a chave pública $K^{+}_{\servconst_{\servi}}$ de \servs através do certificado digital disponível na URI especificada no parâmetro \textit{f} da descrição da mídia, como ilustrou-se no Algoritmo~\ref{algo:sdp-mediadesc}, Linha~\ref{line:sdp-mediadesc:f}, da Seção~\ref{subsubsec:desc-conteudo}. Isto ocorre após o nó \repasss receber o pacote \pac{GMTP-Register-Reply}, que confirma o registro de participação ou a conexão para obter um fluxo de dados \setpk, como apresentou-se no Algoritmo~\ref{algo:registerRelay}, Linha~\ref{algo-line:respondToClients1}, Seção~\ref{subsec:registro-participacao}.
