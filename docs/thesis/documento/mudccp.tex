...
Na Seção~\ref{subsec:conexao-requisicao}, discute-se mais detalhes sobre o envio
do pacote do tipo \pac{GMTP-MediaDesc}.
...
\clis $\in$ \subsetcli$($\repass$)$ em sua rede local recebendo \setpk, através
de um nó \repass. Já a segunda fase acontece quando um novo nó \cliu{2} $\in$
\subsetcli$($\repass$)$ deseja obter o mesmo fluxo de dados \setpks que o nó
\cliu{1} solicitou previamente. E, por fim, a terceira fase acontece quando o nó
\repasss começa a buscar novos nós parceiros \repassu{q} a fim de obter
\setpks através de fontes alternativas.

Para entender o funcionamento de cada fase do processo de conexão do GMTP,
considere a Figura~\ref{fig:processo-conexao}. Nesta figura, ilustra-se um nó
\serv, que gera um fluxo de dados \setpk, conectado a uma rede com 12 nós
\repass, dos quais 1 funciona como repassador de uma rede local, composta por 6
nós \clis $\in$ \subsetcli$($\repass$)$.
...
Figura~\ref{fig:tabela-recepcao-1}. A tabela de recepção de fluxos de
dados serve para que um nó \repasss registre todos os fluxos de dados que estão
sendo repassados para seus nós \clis $\in$ \subsetcli$($\cli$)$, mantendo-se as
...
Um nó \repasss deve consultar a tabela de recepção de fluxo de dados todas as
vezes que receber um pacote do tipo \pac{GMTP-Request}, transmitido por um nó
\clis $\in$ \subsetcli$($\repass$)$. Com base no estado da referida tabela, o nó
\repasss realiza uma determinada ação de repasse, que dependerá de qual fase
de conexão que o referido nó repassador estará com relação a um determinado
fluxo de dados \setpk.

Note que, independente da fase de conexão em que um nó \repasss se encontra
para obter um fluxo de dados \setpk, o estabelecimento de uma conexão sempre
se inicia no nó \cli, quando um usuário final de uma aplicação em execução no
referido nó deseja obter \setpks e reproduzi-lo. A seguir, discutem-se esssas
e outras questões relacionadas às fases do processo de estabelecimento de
conexão do GMTP.

\subsection{Fase 1: primeira requisição a um fluxo de dados \setpk}
\label{subsec:conn-fase1}

Com base na Figura~\ref{fig:processo-conexao-1}, onde se ilustra um exemplo
...
\subsetcli$($\repassu{1}$)$; e
  ...
  \item \cliu{3}, um nó cliente que deseja obter um fluxo de dados \setpk, tal
que \cliu{3} $\in$ \subsetcli$($\repassu{1}$)$.

\end{itemize}

Para obter o fluxo de dados \setpk, o nó \cliu{3} cria um socket GMTP e
transmite um pacote do tipo \pac{GMTP-Request}
(Figura~\ref{fig:processo-conexao-1}, Passo 1).
Para construir o pacote do tipo \pac{GMTP-Request}, qualquer nó \clis deve
obrigatoriamente especifica o valor para o endereço IP de destino o endereço do
nó \servs que transmite \setpk, com o valor para o campo do cabeçalho de rede
...
campo \textit{nome do fluxo de dados} (\textit{flow name}), presente no
cabeçalho de transporte do pacote do tipo \pac{GMTP-Request}. Definir o endereço
IP de destino para o IP de \servs permite o roteador padrão obter o endereço IP
do nó \servs que gera o fluxo de dados \setpk, sendo desnecessário realizar tal
consulta no servidor DNS, como discutiu-se na Seção~\ref{subsec:content-index}.
Além disso, com o valor de \textit{TTL=1}, permite-se interceptar o referido
pacote de requisição, evitando-se extrapolar o domínio administrativo de tal
rede local.
...
Como na Fase $1$ de estabelecimento de conexão para obter um fluxo de dados
\setpk não existe nenhum nó \clis recebendo o referido fluxo, então quando o
pacote \pac{GMTP-Request} alcançar o nó \repassu{1} (Passo 2 da
Figura~\ref{fig:processo-conexao-1}), o estado da sua tabela de recepção de
fluxos de dados não conterá um registro para o fluxo de dados \setpk, tal como
ilustrou-se na Figura~\ref{fig:tabela-recepcao-1}. Nesse instante, o nó \repasss
então inicia um processo de registro de participação para obter o fluxo de dados
\setpk, tal como descreveu-se na Seção~\ref{subsec:registro-participacao},
resultando na chamada do procedimento \textit{registerRelay(\serv, \pk)}, onde
\pks é do tipo \pac{GMTP-Request}. Como resultado da invocação do procedimento
de registro de participação, o nó \repassu{1} transmite o pacote do tipo
\pac{GMTP-Register} e, à medida que os roteadores da rede repassam o pacote
\pac{GMTP-Register} até alcançar o nó \serv, constitui-se o caminho \setwayiu{1}
$=$
$\{$\repassu{1},\repassu{2},\repassu{3},\repassu{4},\repassu{5},\servu{1}$\}$,
destacado na cor vermelha da Figura~\ref{fig:processo-conexao-1}, conforme
discutiu-se na Seção~\ref{sec:descparc}.

AKI!!

Considerando-se que se trata da Fase 1,
nenhum nó \ways $\in$ \setwayi, cada
...
\cliu{1} que o fluxo \setpks será iniciado. Neste ponto, conclui-se a fase
$1$ do processo de estabelecimento de conexão e o fluxo \setpks começa a ser
