...
Desta forma, \net representa formalmente a rede de sobreposição constituída
...
empregados do GMTP a fim de construir \net, processo que ocorre em três grandes
fases:
...
com o evento \events a ser transmitido (Seção~\ref{sec:connformnet});
  ...
  \item \textit{Distribuição do fluxo de dados \setpks através de \net:}
transmitir os pacotes de dados \pks $\in$ \setpks através da rede de
sobreposição constituída na Fase 1 (Seção~\ref{sec:asptransrecep}); e

  \item \textit{Controle de congestionamento em \net:} controlar o tráfego de
dados gerado na Fase 2 e transmitido através da rede de sobreposição formada na
Fase 1 (Seção~\ref{sec:ccgmtp}).
...
os pacotes de dados \pks $\in$ \setpk sejam entregues o mais rápido possível ao
...
da rota de rede utilizada para transmitir \setpk, consegue-se distribuir \setpk
para mais nós \clis sem que seja necessária múltiplas conexões em \serv. Além
disso, por possuir mais nós \repass, um caminho completo é mais tolerável à
desconexões.
...
funcionar como um nó repassador de um ou mais fluxos de dados \setpk.

Para realizar um registro de participação, um nó \repasss envia uma mensagem
para um nó \servs utilizando o pacote \pac{GMTP-Register} e este envio permitirá
a descoberta de um caminho \setwayi. Isto porque todos os nós \repasss no
caminho entre \repasss e \servs devem alterar o pacote \pac{GMTP-Register},
adicionando seu identificador que, na prática, pode ser o endereço IP. Quando o
pacote \pac{GMTP-Register} alcançar o destino \serv, \servs terá a lista
ordenada dos nós \repasss até \servs e a armazenará como sendo um dos possíveis
caminhos para distribuir um fluxo de dados \setpk. Como resposta para \repass,
o nó \servs deve enviar um pacote do tipo \pac{GMTP-Register-Reply}, confirmando
o registro e informando o caminho \setwayis que acabara de ser descoberto.
Posteriormente, \setwayis será utilizado no processo de formação de parcerias, a
ser discutido mais adiante na Seção~\ref{sec:descparc}.
...
dados \setpks. Mesmo assim, um registro de participação pode ocorre no mesmo
...
\repasss não é obrigado a informar qual evento \event está interessado. Com
isso, ao final do procedimento do registro de participação, o nó \repasss passa
a conhecer sobre \setwayis através do pacote \pac{GMTP-Register-Reply} (linha
\ref{ln:parsepath}). Note também no GMTP, toda transferência de pacotes de
controle ocorre com garantia de entrega, representando-se tais ações pelas
funções com nomes contendo o sufixo \textit{Rdt} (\textit{Reliable data
transfer}). Uma outra decisão importante tomada no GMTP é que um nó \repasss
deve periodicamente sinalizar sua participação na rede de favores \nets através
de um modelo de \textit{keep-alive} tradicional, comumente utilizado em outros
protocolos de rede consolidados, como o TCP. Nesse aspecto, o GMTP segue a RFC
1122, \textit{Requirements for Internet Hosts - Communication
Layers}~\cite{RFC1122}.
...
Além disso, um nó \repasss também pode sinalizar explicitamente sua desconexão a
\serv, quando não desejar mais participar da rede de favores $\eta$. Para isto,
deve-se enviar um pacote do tipo \pac{GMTP-Close}. Em qualquer um dos casos de
desconexão, por expiração do tempo ou explícita, o nó \servs deve desconsiderar
\repasss no processo de formação de parcerias e enviar para o nó \repasss um
pacote do tipo \pac{GMTP-Reset}.\\
...
\If(){pkt = OK}{
  ...
  \uIf{pkt = OK}{
    ...
    \setwayis \attrib \parsePath(pkt)\tcc*[r]{get the \setwayis until
\serv} \nllabel{ln:parsepath}
    \If(){\clis $\neq$ NULL}{
      \tcc{notify \clis that \repasss is registered in \serv}
      pkt \attrib \makePkt{GMTP-RequestNotify(1), \cli}\;
      \sendPktRdt{pkt}\;
    }
  }
...
O registro de participação do GMTP permite que quanto mais nós \repasss se
registrarem em nós \serv, mais caminhos \setwayis serão conhecidos.
Consequentemente, quanto mais caminhos forem conhecidos, mais parcerias poderão
ser formadas entre os nós \repass. Portanto, quanto mais parcerias formadas,
maior será o número de nós \clis capazes de receber um fluxo de dados \setpks
originado em \serv, disponibilizado indiretamente através dos seus respectivos
nós \repass, sem nenhuma influência da camada de aplicação.

Por fim, no mundo real (Internet), os nós \repasss podem passar a constituir
...
roteador para registra-lo em múltiplas redes de distribuição. Nesses casos, as
redes de distribuição podem fazer uso de tal roteador em momentos ociosos com
relação a utilização da largura de banda disponível para a conexão do roteador
do usuário à Internet. Como consequência, relações comerciais podem ser
construídas, mas discussões nesse âmbito não são realizadas no contexto deste
nesse trabalho.
...
parceria por intersecção ocorre quando um nó
\repasss transmite um pedido de registro de participação ou um pedido de
conexão para obter um fluxo de dados \setpk, através de um caminho \setwayis que
já esteja repassando \setpk. Assim, após o registro de participação de
\repassu{9} e a consequente constituição de \setwayiu{1} (Passo 1), se qualquer
um dos nós \repassu{10..12}, suponha \repassu{11}, enviar um registro de
participação ou um pedido de conexão para obter \setpks (Passo 2 da
...
\SetAlgoLined
\tcc{This function finds the first node \ways common to a known path \setwayis and the path \setwayiu{\repassconst_\repassi}. The path \setwayis contains the node \way, which routes a given flow \setpks to another node \wayu{q} $\in$ $\delta($\way, \setwayi$)$. The path \setwayiu{\repassconst_\repassi} and info about the flow \setpks are available in the packet \pk, which contains all the node \ways along the path between \repass (requester) and the node \serv, which executes this procedure.}
\textit{done} \attrib \textit{false}\tcc*[r]{It becomes true when \ways is found}

\SetKwFunction{Union}{Union}\SetKwFunction{makePkt}{makePkt}
\SetKwFunction{Union}{Union}\SetKwFunction{extractAttribute}{extractAttribute}
\SetKwFunction{Union}{Union}\SetKwFunction{recvPktRdt}{recvPktRdt}
\SetKwFunction{Union}{Union}\SetKwFunction{sendPktRdt}{sendPktRdt}
\SetKwFunction{Union}{Union}\SetKwFunction{return}{return}
\SetKwFunction{Union}{Union}\SetKwFunction{length}{length}
\SetKwFunction{Union}{Union}\SetKwFunction{getKnownPathsOfFlow}{getKnownPathsOfFlow}

% \textit{\setpk} \attrib \extractAttribute{\pk, `flow'}\tcc*[r]{Extracts \setpks in \pk}
% \textit{\setwayiu{\repassconst_\repassi}} \attrib \extractAttribute{\pk, `path'}\tcc*[r]{Extracts \setwayiu{\repassconst_\repassi} in \pk}
...
\ForEach{\setwayis $\in$ \setwayiu{\setpkc}}{
	...
	\tcc{The node \ways is the first common relay of a known path \setwayis and the new one \textit{\setwayiu{\repassconst_\repassi}}.}
...
%       \pks \attrib \makePkt{\pac{GMTP-Response}(1), \ways}\;
      \tcc{\servs registers \textit{\setwayiu{\repassconst_\repassi}} as a known path and must send a response to \repasss asking \ways to act as a relay for the flow \setpks and.}
...
\caption{matchPartnersByPathIntersection(\repass: PeerRelay, \pk: \pac{GMTP-Request})}
\end{algorithm}
\vspace{0.8cm}
...
O procedimento de formação de parcerias por combinação, como o próprio nome o
define, consiste em combinar dois (ou mais) caminhos distintos \setwayiu{1} e
\setwayiu{2} tal que \setwayiu{1} $\cap$ \setwayiu{2} $=$ \{\serv\}, ou seja, os
caminhos se interceptam apenas no nó \servs, como ilustra-se no Passo 1 da
Figura~\ref{fig:esquema-abstrato-formacao-parceria-combinacao}). Dessa forma,
este procedimento considera um nó \textit{pivot} \servs e o conjunto \setways de
