...
as principais estruturas adotadas nos sistemas de distribuição de mídias ao vivo e as funções básicas empregadas nesses sistemas. Por fim, como complemento, discute-se brevemente sobre criptografia e assinatura digital, compreendendo alguns conceitos aplicados ao GMTP para evitar ataques de poluição. Caso o leitor se considera familiarizado com esses assuntos, recomenda-se, pelo menos, a leitura do sumário deste capítulo, disponível na Seção~\ref{sec:fundamentacao-sumario}. Caso necessário, sugere-se as seguintes referências complementares:
...
Overcast~\cite{jannotti_overcast:_2000}, utilizam esta abordagem. Na
...
Trabalhos recentes, como o disponível na referência~\cite{magharei_mesh_2007},
...
recursos do conjunto $P = p_1, p_2, ..., p_n$ de seus nós participantes para
repassar o conteúdo que é transmitido pelo servidor $S$. Cada nó participante
$p_i$ é livre para entrar e sair do sistema a qualquer momento. Tal
...
aleatoriamente. As interações e trocas de dados entre os nós participantes $p_i$
e $p_j$, com $i \neq j$, são normalmente orientadas pelos pedidos de dados e
informações entre $p_i$ e $p_j$. Assim, esse tipo de rede sobreposta do tipo
...
um nó $p_i$ conecta-se a um servidor centralizado de inicialização, denominado
de \textit{bootstrap} $B$ ou rastreador. Na inicialização de $p_i$, o servidor
$B$ envia um subconjunto, dos nós do sistema para o nó $p_i$. Esse
subconjunto é definido por $LPC_i$ ($LPC_i \subseteq P$ e $LPC_i \neq
\emptyset$) e é a lista inicial dos nós candidatos a parceiros do nó $p_i$.
...
servidor $B$~\cite{zhao_gridmedia+:_2009}. Assim, $p_i$ tem uma referência do
...
de $p_i$, denominado $LP_i$ ($LP_i \subseteq LPC_i$). Os conjuntos $LPC_i$ e
$LP_i$ são dinâmicos, pois cada nó $p_j$ está livre para abandonar o sistema.
Quando $p_j \in LP_i$ e o nó $p_i$ detecta a inatividade deste parceiro, $p_i$
remove $p_j$ de $LPC_i$ e $LP_i$ e seleciona um novo elemento de $LPC_i$ para
criar uma nova parceria.

O nó $p_i$ sempre tenta manter sua $LPC_i$ com um número de candidatos acima de
...
$|LPC_i| < L_i$, então $p_i$ recorre ao servidor $B$ para obter novos elementos
para $LPC_i$.

Cada nó $p_i$ também contém um mapa de partes da mídia de tamanho \textit{m},
representado por $cm_i$. Esse mapa sinaliza os \textit{chunks}
da mídia que o nó $p_i$ contém ou necessita. Ou seja, o mapa $cm_i$
representa um trecho contínuo da mídia transmitida ao vivo pelo sistema P2P que
será reproduzida pelo nó $p_i$.
...
Periodicamente, $p_i$ verifica quais \textit{chunks} $c_t$ são necessários
...
\multicolumn{1}{|>{\columncolor[rgb]{0.913,0.929,0.957}}l|}{$P = p_1, p_2,..., p_n$} &
\multicolumn{1}{>{\columncolor[rgb]{0.913,0.929,0.957}}l|}{Conjunto dos nós
participantes do sistema.}
	    \\

\multicolumn{1}{|>{\columncolor[rgb]{0.66,0.7,0.759}}l|}{$p_i$} &
...
\multicolumn{1}{>{\columncolor[rgb]{0.913,0.929,0.957}}l|}{Número mínimo de nós candidatos $p_i$.}
	    \\

\multicolumn{1}{|>{\columncolor[rgb]{0.66,0.7,0.759}}l|}{$cm_i$} &
\multicolumn{1}{>{\columncolor[rgb]{0.66,0.7,0.759}}l|}{Mapa de \textit{chunks} de $p_i$.}
	    \\

\multicolumn{1}{|>{\columncolor[rgb]{0.913,0.929,0.957}}l|}{$cm_i[x] = desejada$, onde $x = [0..m]$} &
\multicolumn{1}{>{\columncolor[rgb]{0.913,0.929,0.957}}l|}{Conteúdo inicial do mapa de \textit{chunks} de $p_i$.}
	    \\
...
\multicolumn{1}{>{\columncolor[rgb]{0.913,0.929,0.957}}l|}{Mapa de \textit{chunks} de $p_i$.}
	    \\

\multicolumn{1}{|>{\columncolor[rgb]{0.66,0.7,0.759}}l|}{$cm_i[x] = desejada$, onde $x = [0..m]$} &
\multicolumn{1}{>{\columncolor[rgb]{0.66,0.7,0.759}}l|}{Conteúdo inicial do mapa de \textit{chunks} de $p_i$.}
	    \\
            \hline
...
ser contatado por um outro nó participante $p_j$, requisitando sua parceria ou
...
parceiros para aumento da conectividade, o nó $p_i$ recorre à lista de nós
...
$p_k$, como uma escolha aleatória entre os nós pertencentes a $LPC_i$, com base
nos recursos disponíveis em $p_k$, proximidade temporal ou até mesmo proximidade
geográfica, informados por $B$ ou por comunicação direta entre os parceiros.

Uma vez selecionado o nó candidato, $p_i$ envia uma mensagem de pedido de
parceria ao nó candidato $p_k$, selecionado de $LPC_i$. Caso $p_k$ tenha
recursos disponíveis (por exemplo, banda de rede, conexões disponíveis na
aplicação etc.), este adiciona $p_i$ à $LP_k$ e responde a solicitação de $p_i$.
Quando $p_i$ recebe a resposta de $p_k$, este adiciona $p_k$ à $LPC_i$ e a nova
parceria é de fato estabelecida. No momento que um nó $p_i$ adiciona um novo nó
parceiro $p_k$ à $LPC_i$, este inicia um temporizador $t_{ik}$, o qual é
acionado em intervalos de tempo $tp_i$. A cada expiração do temporizador
$t_{ik}$, o nó $p_i$ envia uma mensagem ao nó parceiro $p_k$ para verificar seu
...
\textit{chunk} de interesse $c_j$. Quando $p_k$ recebe um pedido por dados vindo
de um parceiro ativo $p_i \in LP_k$, este cria uma mensagem de resposta contendo
o \textit{chunk} $c_j$ requisitado e o envia ao nó $p_i$. Caso o pedido seja
enviado por um nó não parceiro, ou seja, $pi \notin LP_k$, $p_k$ ignora o pedido
por $c_j$, mas adiciona $p_i$ à lista de nós parceiros candidatos $LPC_k$. O nó
$p_i$ espera a resposta de $p_k$ acerca do pedido de parceria por um
intervalo de tempo $tp_i$. Caso $p_k$ não responda no intervalo de tempo $tp_i$,
$p_i$ remove $p_k$ de sua $LP_i$. Caso $p_i$ receba a resposta de $p_k$, $p_i$
atualiza os dados relativos a $p_k$, como por exemplo, o mapa de \textit{chunks}
$cm_k$. Se o pedido pelo \textit{chunk} $c_j$ for respondido durante o intervalo
de tempo esperado, $p_i$ coloca o novo dado em seu \textit{buffer} de reprodução
