...
if opt == "1" or not opt: #generate
	...
	repeticao_array = [68, 62, 63, 71, 57, 60, 52, 50, 51, 54, 72, 69, 57, 66, 51, 77, 63, 55, 55, 52, 50, 55, 59, 61, 60, 50, 52, 73, 76, 65, 65, 66, 52, 79, 74, 54]
	...
	for tratamento in range(0, 18):
		...
		for ensaio in range(0, int(repeticao)):
			# GMTP
			startup_delay_gmtp = "%d.%d" % (random.randrange(1, 100), random.randrange(1, 99)) 
...
#			print startup_delay_gmtp, "\t", cont_index_gmtp, "\t", distortion_gmtp, "\t", server_conn_gmtp, "\t", client_conn_gmtp, "\t", ctrl_packets_gmtp, "\t"
			...
			# DENACAST
			startup_delay_denacast = "%d.%d" % (random.randrange(1, 100), random.randrange(1, 99))
