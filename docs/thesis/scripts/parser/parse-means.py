...
if opt == "1": #load means
    ...
    if subopt == "3":
        ...
        for tratamento_i in range(0, len(tratamentos_array)):
            ...
            file_tabela_latex_gmtp_denacast.write("\multirow{2}{*}{\\textcolor{black}{\\bfseries GMTP}} & $%.2f$ & $%.2f$ & $%.2f$ & $%.2f$ \\\\ \multirow{2}{*}{\\textcolor{black}{\\bfseries %d}} & $%s$ & $%s$ & $%s$ & $%s$ \\\\\n \cline{2-5} \n\multirow{2}{*} {\\textcolor{black}{\\bfseries Denacast}} & $%.2f$ & $%.2f$ & $%.2f$ & $%.2f$ \\\\\n & $%s$ & $%s$ & $%s$ & $%s$ \\\\ \hline\n" % (startup_delay_gmtp_mean, cont_index_gmtp_mean, distortion_gmtp_mean, ctrl_packets_gmtp_mean, tratamento_i+1, startup_delay_gmtp_ci, cont_index_gmtp_ci, distortion_gmtp_ci,  ctrl_packets_gmtp_ci, startup_delay_denacast_mean, cont_index_denacast_mean, distortion_denacast_mean, ctrl_packets_denacast_mean, startup_delay_denacast_ci, cont_index_denacast_ci, distortion_denacast_ci, ctrl_packets_denacast_ci))
            file_tabela_latex_gmtp_ccntv.write("\multirow{2}{*}{\\textcolor{black}{\\bfseries GMTP}} & $%.2f$ & $%.2f$ & $%.2f$ & $%.2f$ \\\\ \multirow{2}{*}{\\textcolor{black}{\\bfseries %d}} & $%s$ & $%s$ & $%s$ & $%s$ \\\\\n \cline{2-5} \n\multirow{2}{*} {\\textcolor{black}{\\bfseries CCN-TV}} & $%.2f$ & $%.2f$ & $%.2f$ & $%.2f$ \\\\\n & $%s$ & $%s$ & $%s$ & $%s$ \\\\ \hline\n" % (startup_delay_gmtp_mean, cont_index_gmtp_mean, distortion_gmtp_mean, ctrl_packets_gmtp_mean, tratamento_i+1, startup_delay_gmtp_ci, cont_index_gmtp_ci, distortion_gmtp_ci, ctrl_packets_gmtp_ci, startup_delay_ccntv_mean, cont_index_ccntv_mean, distortion_ccntv_mean, ctrl_packets_ccntv_mean, startup_delay_ccntv_ci, cont_index_ccntv_ci, distortion_ccntv_ci, ctrl_packets_ccntv_ci))
